...
- `Env.throwContext` (`comptime/env.zig`) tracks how `throw` is checked in the
  body being walked: `.result E` (return is `@Result<D, E>`), `.plain`
  (declared non-Result return → `throw` illegal), or `.unchecked` (no declared
  return type / inside a lambda).
- `inferFnDecl` sets/restores it from the fn's return type; `inferFunctionExpr`
  resets it to `.unchecked` so nested lambdas don't see the outer `E`.
- `inferJumpExpr`'s `throw_` arm enforces it: `unify(E, thrown)` for `.result`,
  `throwWithoutResult` error for `.plain`.
- New error kind `throwWithoutResult` in `comptime/error.zig` (+ renderer in
  `comptime/tests.zig`).
- Tests + snapshots in `comptime/tests.zig` cover every scenario below.
