...
fn inferLoopExpr(env: *Env, lp: ast.MakeExpr(.untyped, ast.LoopExprOf(.untyped)), loc: ast.Loc) InferError!TypedExpr {
    ...
    if (lp.kind.label) |lbl| try env.labelStack.append(env.arena, lbl);
...
fn inferFunctionExpr(env: *Env, func: ast.FunctionExprOf(.untyped), loc: ast.Loc) InferError!TypedExpr {
    ...
    return switch (func.kind) {
        ...
        .fnExpr => |f| {
            // An anonymous `*fn(...)` has no declared return type, so we permit
            // both `await` and `yield` (item type unknown ⇒ no unification).
            env.starFn = if (f.isStarFn) .{ .allowsAwait = true, .iterItem = null } else null;
