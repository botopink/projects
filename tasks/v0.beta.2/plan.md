...
| `expr-templates` | comptime template strings: `type`/`expr` meta-kinds, `${…}` interpolation, tagged calls, hygienic splice (html/yaml models) | nothing (front-end collision w/ stdlib-gleam, test-blocks) | ✅ yes — `specs/expr-templates.md` |
...
- **2026-06-04** — Designed `expr-templates` with the user (long keyword iteration): started from `syntaxeparam`/`compvalue` sketch → `syntaxparam` (symmetry with `typeparam`) → explored `type`/`source`/`code`, `expr`, value-with-holes (`-> comptime`) → **settled on two meta-kinds only**: `type` (replaces `typeparam`) and `expr` (unifies input syntax-param, output code-value, and the `expr { … }` literal). Key unlocks: provenance-based hygiene (each expr resolves in its origin scope) and value→expr lifting (a comptime value is a constant expression — yaml needs no quoting). Hard rule from user: **no new `@`-builtins** — data model (`Span`/`Part`/`Binding`) is plain stdlib (`std.syntax`); keywords own the type forms + literal. Canonical models: `html` (scope-aware, bounded `-> expr Component`) and `yaml` (bare `-> expr`, type revealed per call site). Specced + added to README; wave rule: front-end collision with stdlib-gleam/test-blocks.
