...
| [ci-pipelines-green](specs/ci-pipelines-green.md) | `ci-pipelines-green` | `task/ci-pipelines-green` | `.tasks/ci-pipelines-green/` | **done** (meta `origin/feat` ← 7e18bba; bot-lang ← b6afe7c; all 4 sibling lib `feat` pointers landed). Final sweep walked five layers of pre-existing reds: F1+F2+F3 (action versions + npm glob), erlang/wasmtime install + REF=feat + Node 22 + in-tree test-libs.sh wrapper, OTP 27→28 (closes the `if_with_else_branch` BIF-shadow residual), runtime.zig stdout-only RUN LOG contract, wasmtime tarball pin, lib `allow_fail` matrix shapes (erlang + windows-commonJS pre-existing reds tracked as v0.beta.20 specs). **CI-YAML cleanup deferred** to `ci-pipelines-green-tail` (drop the diagnostic artifact step + `ERL_AFLAGS` env). All 7 workflows report `success` on the latest `feat` push (bot-lang/erika/jhonstart/onze/rakun/vscode-extension `test` + meta `hook-integrity`) |
