...
| [ci-tail.md](specs/ci-tail.md) | 2 — 01-cleanup · 02-backends-parity | `task/ci-tail` (partial) | **partial done** — 01-cleanup closed (A0/A4 v19 row → done; B-half AGENTS.md note landed). 02-backends-parity: beam_asm.zig parity comment landed; `shell: bash` added on 4 sibling-lib source-test step; `snap.zig` CRLF + path-sep normalisation landed. **Deferred**: 4 sibling-lib erlang reds → v0.beta.21 (audit in [`sibling-lib-erlang-codegen-reds.md`](specs/sibling-lib-erlang-codegen-reds.md)); windows-2022 `allow_fail` row drops await a CI cycle confirming the fixes. |
