...
| [botopink-install-from-deps.md](specs/botopink-install-from-deps.md) | flat — `bpmp install` from object-form dependencies (CAS + per-project symlinks + lockfile) | `.tasks/install-from-deps-f3/` (F3 closed) | **partial in-progress — F3 DONE** — spec authored (`325472a`); F0/F1/F2/F4/F5/F6 unimplemented. F3 consumer fixture migration **complete (8/8)**: emilia-card (`af1c66d`), erika-linq (`abfcfe4`), jhonstart-{counter,html,todo} (`7a9b0ae`), onze (`ab7788b`), rakun (`ee798f8`), generic-loader-binding (`3aecd65`) — every consumer `botopink.json` under `repository/*/examples/` now ships the new object-form `dependencies` schema. |
