...
| [snap-audit.md](specs/snap-audit.md) | 1 — flat (F0–F5) | `.tasks/snap-audit/` (in-progress) | **F0+F1.c→b done** — F0: `scripts/snap_audit.sh` authored with 4 modes; baseline **2 053** `*.snap.md` (220 parser · 789 comptime · 942 codegen incl. 4 errors · 102 lsp); legacy sweep: 24 `external-legacy` hits (zero of every other surface). F1: 14 `(c)/empty` fixtures promoted to `(b)` with `fn main()` wrapper across 4 backends — node +12 nonempty (55→67), erlang +5 (41→46), beam +11 (50→61), wasm +3 (27→30); 9 `## Findings` rows authored routing to `prim-op` / `frente-a-01-{future,wat-refactor,erika-runtime-string}` / `frente-a-03-closeout` / `ci-tail-02-backends-parity` / `frente-b codegen-break-label`. Remaining F1: `(a)` decl-style promote (141 per backend); F2/F3/F4 untouched. |
