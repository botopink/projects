...
| [test-speed-tmp-consolidation.md](specs/test-speed-tmp-consolidation.md) | flat — F0 runtime.zig path rewrite · F1 .gitignore cleanup · F2 clean-tmp build step · F3 runtime_scratch.zig pin · F4 AGENTS+CHANGELOG | (pending) | **pending** — consolidate per-test scratch dirs from `modules/compiler-core/.tmp-exec-*/` siblings into `modules/compiler-core/.botopinkbuild/tmp/<hex>/`; single callsite change in `runtime.zig:makeScratchDir`; drop dead `.gitignore **/.tmp-exec-*/` rule; 1-day TTL reap via `clean-tmp` step. |
