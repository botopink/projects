...
`repository/rakun/src/file_router.erl`, `repository/rakun/test/file_router_test.bp`
...
This front's registry is separate from `rkRegisterRoute`'s: `file_router.erl` is the BEAM one and is
what serves requests, `file_router.mjs` is the browser one and holds only the table.
...
**The `appDir` is configuration, not a literal.** `rkProp("onze13.appDir")` (front 05) resolves to
...
The scan is the CLI-side half (`file_router.erl` for the server's own startup check, `file_router.mjs`
for `onze13 dev`). It reads `appDir` from front 05, walks the tree, and fails loudly rather than
...
- `file_router.erl` and `file_router.mjs` implement the same registry protocol, and the round-trip
