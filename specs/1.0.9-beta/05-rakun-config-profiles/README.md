...
**Wave:** 1
**Depends on:** 01
**Owns:** `src/config.bp`, `src/config.erl`, `src/config.mjs`, `src/profiles.bp` · `test/config_test.bp`
...
implemented for it. `src/config.mjs` in the ownership row is a mistake — see *Contradictions*.
...
1. **`src/config.mjs` should not exist.** This is a server front on the erlang target and the exit gate
   forbids a `@External.Node` cell in one. The Node half of the property table already exists in the
   frozen `runtime.mjs`; a second Node file here would be new Node surface in a BEAM front.
2. **`src/config.erl` cannot be the sidecar's name.** The atom `config` collides with the emitted
   `rakun/config` module, so `shipErlSidecars` skips it (`libs.zig:596`). The file is
   `src/sidecars/rakun_config.erl`. Same defect as front 04's `src/runtime.erl`.
3. **`src/root.bp` is allocated to nobody and three fronts need a line in it.** Front 05 must add
   `pub mod config;` and `pub mod profiles;`, front 06 `pub mod context;`, `pub mod events;`,
   `pub mod lifecycle;`. Suggest giving `src/root.bp` to front 04 (the earliest of the three) with the
   same "hand it your export lines" rule track A uses for `libs/std/src/root.bp`.
