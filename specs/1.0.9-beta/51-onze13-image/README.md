...
allowlist check, the resize/re-encode call, the cache lookup and the `/_onze13/image` route handler.
...
**Does not touch:** `repository/onze13/src/config.bp` · `types.bp` · `integration.bp` · `root.bp`
(F49 — except the one `pub mod image;` export line, handed to F49), `src/font.bp` (F52),
`repository/onze13/modules/onze13-og/**` (F70), `repository/jhonstart/src/element.bp` (frozen),
...
**Replaces:** `1.0.7-beta/20-onze13-image-optimization`

---

## Problem

An onze13 app serves images the way a 1998 site did: the author writes the path, the browser fetches
...
- `repository/onze13/src/image.bp` does not exist; `repository/onze13/` does not exist until front 49.
...
2. **Compute the optimized URL.** `/_onze13/image?src=<src>&w=<w>&q=<q>&f=<fmt>`, with the parameter
...
`/_onze13/image` is a built-in route registered by the CLI (front 50), not a file in the app's `app/`
tree. It takes `src`, `w`, `q` and `f`, revalidates them against the same rules the component applied
...
### Step 5 — The `/_onze13/image` handler

```bp
#[@future]
pub fn imageHandler(ctx: RouteContext, cfg: ImageConfig, publicDir: string) -> @Future<Response>
```
...
byte-for-byte across backends — that is the cheapest check that the erlang half of onze13 is real. The
