...
equivalent called `onze13`, and [1.0.8-beta](../1.0.8-beta/overview.md) proposed Tailwind CSS parity
...
**Then the three references were read end to end, and fifty-three became ninety-three.** Each of the
...
| The client bundle. Five fronts were already js-target, and nothing computed the module graph, emitted a bundle, or wrote a hydration entry. `onze13 build` would have shipped a server with no browser half. | 68 |
...
| **C — jhonstart** | 26–32 · 67 | `repository/jhonstart` | The React half of Next.js |
| **D — emilia** | 33–48 · 54–59 | `repository/emilia` | Tailwind CSS v4 parity, plus the attribute slot |
| **E — onze13** | 49–53 · 68–71 | `repository/onze13` (new) | The orchestrator, the CLI, and the app that proves it |
...
| [`88-rakun-cli`](./88-rakun-cli/README.md) | medium | rakun's own CLI — distinct from `50-onze13-cli` |
...
| Front | Priority | Description |
|---|---|---|
| [`33-emilia-color-palette`](./33-emilia-color-palette/README.md) | **critical** | 22 families × 11 shades, plus `black`/`white` and opacity |
...
### Track E — onze13 (49–53 · 68–71)

| Front | Priority | Description |
|---|---|---|
| [`49-onze13-stand-up`](./49-onze13-stand-up/README.md) | **critical** | The new repository: manifest, module tree, shared types, integration layer |
| [`50-onze13-cli`](./50-onze13-cli/README.md) | medium | `create`, `dev`, `build`, `start` |
| [`51-onze13-image`](./51-onze13-image/README.md) | low | The `Image` component and the optimizer behind it |
| [`52-onze13-font`](./52-onze13-font/README.md) | low | Font loading without layout shift |
| [`53-onze13-example-app`](./53-onze13-example-app/README.md) | medium | The blog that exercises every front above |


| [`68-onze13-client-bundle`](./68-onze13-client-bundle/README.md) | **critical** | The `'use client'` module graph, the browser bundle, the hydration entry, the secret-leak refusal |
| [`69-onze13-styling-pipeline`](./69-onze13-styling-pipeline/README.md) | medium | Where `emilia.flush()` output is collected and inserted during the server render |
| [`70-onze13-image-response`](./70-onze13-image-response/README.md) | low | Programmatic OG image generation |
| [`71-onze13-release-packaging`](./71-onze13-release-packaging/README.md) | medium | Self-hosting: OTP release, container image, standalone output |
## Order
...
Wave 0 — nothing blocks these
  ...
  03-std-content-hash                 │   01 lands last: its `root.bp` commit
                                      │   carries their exports)
  54-emilia-theme                     │  (54 and 56 come before 33/34/35:
  56-emilia-cascade-and-output        │   the palette, the modifiers and the
  33-emilia-color-palette             │   scale all emit through them)
  34-emilia-modifiers                 │
...
Wave 3 — what stands on them  ▼
  ...
  29 ──► 68-onze13-client-bundle           16 ──► 84
...
Wave 4 — the last dependents  ▼
  24 + 68 ──► 67-jhonstart-forms
                              │
Wave 5 — the proof            ▼
  53-onze13-example-app
```
Wave 0 — nothing blocks these
  01-std-lib-enablement ──────────────┐
  02-std-async-primitives             │  (02, 03 need nothing from 01, but
  03-std-content-hash                 │   land with it to keep std one review)
  ...
  49-onze13-stand-up                  │
                                      │
Wave 1 — the foundations              ▼
  04-rakun-erlang-runtime ────┐   36..47-emilia-* (all of them, in parallel)
  05-rakun-config-profiles ───┤
  22-rakun-file-routing ──────┤
  26-jhonstart-router ────────┘
                              │
Wave 2 — the services         ▼
  06-rakun-context-api ──► 07 · 08 · 11 · 12 · 13 · 14 · 15 · 16 · 17 · 19 · 21
  22 ──► 23-rakun-ssr-pipeline
  26 ──► 27-jhonstart-link · 28-jhonstart-server-components
                              │
Wave 3 — what stands on them  ▼
  07 ──► 09 · 10 · 18 · 20        23 ──► 24 · 25
  28 ──► 29 · 30 · 31 · 32        26 + 48 ──► 50 · 51 · 52
                              │
Wave 4 — the proof            ▼
  53-onze13-example-app
```
...
**Why `62-rakun-request-context` and `68-onze13-client-bundle` are on the critical path.** The audit
...
- **The compiler knows none of this.** `rakun`, `jhonstart`, `emilia` and `onze13` are libraries.
...
- **Additive only, with one named exception.** No existing `emilia` token is renamed, no existing
  ...
  it says so in its README and covers the old and new output in one test.
...
| C — jhonstart | 26 · 28 · 30 · 31 · 32 | **erlang** — these render on the server |
| C — jhonstart | 27 · 29 | **js** — these run in the browser |
| D — emilia | 33–48 | comptime; 48 emits attributes both halves read |
| E — onze13 | 49–53 | orchestrator: serves from BEAM, ships a JS bundle |

## Mapping the three sources

| Upstream | Reference | Front |
|---|---|---|
| Spring Boot 4 | `/home/ericfillipe/develop/spring-boot-4/docs/` | 04–21 |
| Next.js | `/home/ericfillipe/develop/nextjs/NEXTJS-DOCS.md` | 22–32, 49–53 |
| Tailwind CSS v4 | `/home/ericfillipe/develop/tailwindcss/TAILWIND_CSS_DOCS.md` | 33–48 |
