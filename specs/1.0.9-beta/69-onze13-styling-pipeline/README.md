# Front 69 — onze13 Styling Pipeline
...
`path`)
**Owns:** `repository/onze13/modules/onze13-assets/src/**`,
`repository/onze13/modules/onze13-assets/test/**`
**Does not touch:** `repository/emilia/src/**` (emilia's `flush()` contract is consumed, never
changed), `repository/onze13/modules/onze13-bundler/src/manifest.bp` (front 68 owns the record and
the parser; this front hands it the style records), `repository/onze13/src/integration.bp` (front 49),
`repository/rakun/src/ssr.bp` (front 23)
**Reference:** `NEXTJS-DOCS.md § 15. Estilização (CSS)` (CSS Modules · Global CSS · Sass ·
`useServerInsertedHTML`), `§ 3. Estrutura do Projeto` (`public/`), `§ 13. Streaming` ·
...
- `repository/onze13/` does not exist; `modules/onze13-assets/` is created by this front inside it.
...
```bp
pub type StyleChunk(id: string, css: string, inline: bool)
...
#[@future]
pub fn collectChunk(sink: StyleSink, chunkId: string) -> @Future<#(StyleSink, string)>
...
| `collectChunk(sink, id)` | after a streamed boundary has been rendered and before that boundary's markup is written to the wire | `<style data-onze13-chunk="<id>">…</style>`, or `""` when the chunk registered nothing |
...
The class in the tree's `attrs` is whatever `emilia(tokens)` returned during **this** render. The
sink never recomputes a name, never re-hashes a rule body, never deduplicates by content and never
rewrites a class. It copies the block emilia produced, verbatim, into the document. That is what
makes this front and front 48 independent: front 48 decides how a class reaches `attrs`, emilia
decides what the class is called, and this front decides only *where the block goes*.
...
`data-onze13-chunk`, `href`, `rel` — go through front 01's `escape.attribute`.
...
```
app/blog/blog.module.css   →   .onze13/styles/app_blog_blog.bp
```
...
emitted as one fingerprinted file `/_onze13/static/<buildId>/app.<hash>.css`. It is a `<link>` in the
...
|---|---|---|
| `/_onze13/static/<buildId>/…` — fingerprinted | `public, max-age=31536000, immutable` | yes |
...
- [ ] `collectChunk` returns `""` for a chunk that registered nothing
...
```bp
pub fn chunkAttr(chunkId: string) -> #(string, string)
```
...
- [ ] `chunkAttr` runs its value through `escape.attribute`
...
`repository/onze13/modules/onze13-assets/test/` — four files, `botopink test --target erlang` from
...
| `assets_test.bp` | `resolveAsset`'s traversal cases, the two cache policies, the content-type table, `304` |
...
- [ ] `repository/onze13/modules/onze13-assets/` exists with `botopink.json`, `src/root.bp`,
...
- [ ] `repository/onze13/docs.md` carries the ordering rule and the four-call sequence verbatim,
