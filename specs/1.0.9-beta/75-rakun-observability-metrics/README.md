...
pub fn summary(name: string, tags: Tag[]) -> Summary;
...
One near miss, recorded because it decides the tracing design: a decorator cannot wrap the body of the
declaration it annotates, so there is no `#[timed]` and no `#[observed]`. Front 12 owns that gap and
states it in full; this front avoids it rather than restating it, by instrumenting at the framework
boundary — where Spring's automatic instrumentation happens anyway — and by giving explicit
instrumentation the combinator form `timed(name, tags, { -> … })`, which is also how
`09 § Registrando Metricas Customizadas` writes it.
