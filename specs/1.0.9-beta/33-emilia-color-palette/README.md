...
- **The theme block is a separate, one-time artifact.** `paletteVar(family, shade)` builds the
  custom-property name; a `themePreamble()` fn emits the `@theme` block that defines them. The
  numeric OKLCH values in that block come from upstream `theme.css`, not from this spec — the
  reference prints only two of them (`§ 3.6`). See *Reference gaps*.
...
`themePreamble() -> string` emits the `@theme { … }` block that defines every
`--color-<family>-<shade>` the tokens reference. It is a separate public fn, not part of `emilia()`,
because a project that already imports Tailwind's own theme must not get a second copy.

**Acceptance:**
- [ ] `--color-red-500` is defined as `oklch(0.637 0.237 25.331)` — the value the reference prints (`§ 3.6`)
- [ ] `--color-blue-500` is defined as `oklch(0.623 0.214 259.815)` — likewise
- [ ] `themePreamble()` emits 286 custom properties plus `--color-white` and `--color-black`
- [ ] every other numeric value in the preamble is transcribed from upstream `theme.css`, with the upstream commit recorded in the test file's header — see *Reference gaps*
- [ ] `emilia(tokens)` output contains no `@theme` block: the preamble is opt-in
...
### Step 5 — the escape leaf stays a top-level variant

`Color.Hex(value: string)` is declared and matched today but cannot be constructed. This front does
not delete it — deleting it is not additive — but it also does not build on it. Anything that must
carry a string (an arbitrary colour, a `color-mix()` the tokens cannot express, a P3 value) belongs
in a **top-level** `Token` variant, which is the shape the compiler can construct. The escape-hatch
front the coordinator has allocated owns that variant; this front's only obligation is to leave room
for it: `paletteVar` takes and returns plain strings, so a string-carrying variant can reach the
same formatter without a second palette table.

**Acceptance:**
- [ ] `colorTokenToCss`'s `Hex(value)` arm is retained unchanged
- [ ] `paletteVar(family: string, shade: string) -> string` is `pub` so the escape front can call it
- [ ] the README of this front records that `.Color.Hex("#abc")` is unconstructible today, with the
      exact compiler error text
...
5. **Theme preamble anchors** — `themePreamble().contains("--color-red-500:oklch(0.637 0.237 25.331)")`
   and the blue equivalent.
...
- [ ] `themePreamble()` emits the full `@theme` block, with the two reference-documented values exact
