...
**A signal is a host-level throw, not a returned sentinel.** `rakun_navigation:signal/3` calls
`erlang:throw({rakun_nav, Kind, Location, Status})`. The render supervisor — front 23's pipeline, front
24's action dispatcher, front 25's handler wrapper — brackets the body with a catch that recognizes
exactly that tuple and re-raises anything else unchanged.
...
derived correctly; that a post-flush signal degrades instead of lying about the status; and that the
wire format round-trips, including a destination containing `|`.
...
- The four-line wire format and the redirect-validation rule are written down here once and cited by
  fronts 24, 25, 26, 31 and 64 rather than re-derived.
