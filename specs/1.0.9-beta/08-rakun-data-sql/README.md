...
`#[healthIndicator("db")]` on a component that checks out a connection, runs the driver's liveness
...
1. **`modules/rakun-data/botopink.json` and `src/root.bp` are unallocated.** They belong to F08 as the
   module's first front, and F09 must not create a second manifest.
