...
**Wave:** 1
**Depends on:** 33 (`Text.Decoration.Color` reuses the palette helper)
...
- **A theme variable.** Family, size, tracking and leading all reference `var(--…)` in v4.3, the
  same two-layer shape front 33 uses for colour. The `@theme` values are front 33's
  `themePreamble()` extended with the `--text-*`, `--font-*`, `--tracking-*` and `--leading-*`
  namespaces `§ 3.5` lists; `§ 21.3` and `§ 9.9`/`§ 9.11` print their values, so unlike the colour
  grid this preamble is fully specifiable from the reference.
...
- [ ] the two affected assertions in `src/emilia.bp` are updated, and `grep -R 'font-size:1.125rem' repository/emilia`
      returns nothing outside the `@theme` preamble

### Step 2 — widen size, weight and family

Five sizes (`X5xl` … `X9xl`) and four weights (`Thin`, `Extralight`, `Semibold`, `Extrabold`).
The `--text-*` and `--font-weight-*` theme entries are added to the preamble front 33 owns;
`§ 21.3` prints every size and its line-height, so this half of the preamble is fully specifiable.

**Acceptance:**
- [ ] thirteen sizes, `Xs` through `X9xl`
- [ ] the preamble defines `--text-xs: 0.75rem` with `--text-xs--line-height: calc(1 / 0.75)`, and
      `--text-5xl: 3rem` with line-height `1` — the two shapes `§ 21.3` distinguishes
...
- [ ] the preamble carries the six tracking and five leading values `§ 9.9` and `§ 9.11` print in
      parentheses
...
- [ ] `.Text.Indent.8` uses `spacingScale`
...
3. **Size** — thirteen asserts on the paired-declaration form, plus two preamble asserts covering
...
- [ ] the `--text-*`, `--font-weight-*`, `--tracking-*` and `--leading-*` theme entries are added to
      front 33's preamble, with the values `§ 21.3`, `§ 9.9` and `§ 9.11` print
