...
**Wave:** 1
**Depends on:** 33 (nothing structural; the examples use colour tokens), 35 (the `Inset` scale is the spacing scale and calls `spacingScale`)
...
- **Numeric leaf → a computed length.** `Inset` reuses front 35's `spacingScale`, so
  `.Layout.Inset.T.4` and `.Pad.T.4` agree by construction rather than by coincidence. `Z` is a
  plain integer, not a length.
...
- [ ] the inset scale and the padding scale produce identical length text for the same leaf,
      asserted directly: `.Layout.Inset.T.4` and `.Pad.T.4` differ only in the property name
...
gets its own fn; the `Inset` family gets one fn per direction plus a shared scale fn that delegates
to front 35's `spacingScale`.

**Acceptance:**
- [ ] `layoutTokenToCss` follows the file's `val out = case …; return out;` idiom
- [ ] every arm is an arrow arm
- [ ] `spacingScale` is imported, not copied — there is one spacing scale in the library
...
- [ ] `Inset` shares front 35's `spacingScale`
