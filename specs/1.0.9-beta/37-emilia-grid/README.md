...
**Wave:** 1
**Depends on:** 35 (`Gap` and `Flex.Basis` use `spacingScale`)
...
Two numeric families reuse front 35's `spacingScale`: `Gap` and `Flex.Basis`. `§ 6.15` prints
`gap-1` as `calc(var(--spacing) * 1)` and `§ 6.1` prints `basis-1` the same way, so sharing the fn
is not a convenience — it is what makes them agree.
...
- [ ] `spacingScale` is imported from front 35's block, not copied

### Step 5 — the three dispatchers and the top-level arms

`flexTokenToCss` is widened; `gridTokenToCss` and `gapTokenToCss` are new. Two arms are added to the
top-level `tokenToCss` case (`Grid`, `Gap`); `Flex` already has one.
...
- [ ] `gridRepeat` and `spacingScale` are each spelled once
