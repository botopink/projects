...
**Does not touch:** `src/decorators.bp`, `src/http.bp`, `src/bootstrap.bp`, `src/runtime.mjs` — frozen for the milestone. It does not write to `repository/onze13/` either; front 50 owns that CLI and this one never reaches across.
...
**This is rakun's CLI, not onze13's.** Front 50 delivers `onze13`'s CLI — `create`, `dev`, `build`,
...
project whose dev command is `onze13 dev`, generated only when onze13 is available) and `library` (a
...
- [ ] `--template full-stack` generates a project whose documented dev command is `onze13 dev`, and refuses with a usage error when onze13 is not available.
...
| | `rakun` (this front) | `onze13` (front 50) |
|---|---|---|
| Project shape | a BEAM server: controllers, services, data | a full-stack app: routes, server components, a client bundle |
| `new` / `create` | `rakun new` — server only | `onze13 create` — server plus client |
| Dev loop | `rakun run --watch`, hot-loading BEAM modules | `onze13 dev` — the browser, the bundler, and rakun's run path underneath |
| Build | `rakun build` — an OTP release | `onze13 build` — the release plus the client bundle |
| Owns the browser | never | always |

The rule: when both are installed, `onze13` is the entry point and drives rakun's run path; rakun
never starts a bundler, never writes client output, and never becomes a dependency of front 50's
CLI in the other direction. A `--template full-stack` project documents `onze13 dev` as its dev
command precisely so that two tools do not both claim the dev loop.

**Acceptance:**
- [ ] `modules/rakun-cli/` contains no bundler, no asset pipeline and no reference to `repository/onze13/`.
