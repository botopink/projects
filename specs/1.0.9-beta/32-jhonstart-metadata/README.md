...
**Wave:** 3
**Depends on:** 28 · 01 (escaping) · 66 (metadata file routes, read-only)
**Owns:** `repository/jhonstart/src/metadata.bp`, `repository/jhonstart/test/metadata_test.bp`
**Does not touch:** `src/element.bp`, `src/hooks.bp`, `src/html.bp` (frozen), `src/router.bp` (26), `src/link.bp` (27), `src/server.bp` (28), `src/client.bp` (29), `src/suspense.bp`/`src/streaming.bp` (30), `src/error_boundary.bp` (31)
...
- [ ] `src/root.bp` declares `pub mod metadata;` and `botopink.json` lists `metadata.bp`
...
- [ ] `metadata.bp` in the build tree, `root.bp` and `botopink.json` updated
