...
**Replaces:** `1.0.7-beta/21-onze13-font-optimization`
...
- `repository/onze13/src/font.bp` does not exist; `repository/onze13/` does not exist until front 49.
...
2. **An external probe** for a local font, through front 01's process spawner — the same port pattern
   front 51 uses for its encoder. `fc-query` or an equivalent prints the metrics; onze13 parses the
...
points at live on `fonts.gstatic.com`. At build time onze13 fetches the CSS once with a fixed modern
...
pub type Font(
    family: string,       // "Inter"
    className: string,    // "onze13-font-a1b2c3" — apply this to an element
