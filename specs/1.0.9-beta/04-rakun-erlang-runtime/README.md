...
**Wave:** 1
**Depends on:** 01
**Owns:** `src/runtime.erl`, `src/sidecars/runtime.erl`, `src/runtime.bp` (`#[@external(erlang)]` block only) · `test/erlang_runtime_test.bp`
...
1. **`src/runtime.erl` cannot be the sidecar's name.** The atom `runtime` collides with rakun's own
   emitted `rakun/runtime` module and `shipErlSidecars` skips it. The paths this front actually owns
   are `src/sidecars/rakun_runtime.erl` (the host module) and, if a second file is wanted for the
   cowboy adapter seam, `src/sidecars/rakun_cowboy.erl`.
2. **`src/root.bp` needs no change from this front**, but fronts 05 and 06 each need a `pub mod` line
   in it and it is allocated to nobody. See front 05's *Contradictions*.
...
- [ ] `repository/rakun/botopink.json` lists `erlang` in `targets`
