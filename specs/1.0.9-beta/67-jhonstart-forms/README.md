...
**Target:** js (client)
**Wave:** 4
**Depends on:** 24 (the action endpoint and its envelope) · 29 (the client boundary and the hydration entry) · 14 (the constraint set the client mirrors) · 26 (navigation after a submit) · 63 (a redirect returned from an action) · 01 (percent encoding)
...
**Does not touch:** `repository/jhonstart/src/element.bp`, `src/hooks.bp`, `src/html.bp` (frozen for the milestone), `src/router.bp` (front 26), `src/link.bp` (front 27), `src/client.bp` (front 29), `repository/rakun/src/actions.bp` (front 24)
**Reference:** `NEXTJS-DOCS.md § 10. Mutação de Dados` (Formulários · useActionState · Invocando via event handlers · Segurança), `§ 14. Tratamento de Erros` (Erros esperados), `§ 25. Referência de Componentes` (`<Form>`) ·
...
`NEXTJS-DOCS.md § 10` writes `<form action={formAction}>` and `useActionState`, and in this ecosystem
...
Front 67 does not add them: `element.bp` is frozen by `fronts.md`, and where the missing element
constructors come from is being settled by the track-C fronts 26–32. This front consumes whatever
module they land them in and names it in every example.
...
front's, since `useActionState` is what reads it:
...
`ok` and `redirect` come from the envelope, not from `state`. The golden fixture, written here and
asserted by a test on each side of the boundary, is the `state` value:
...
`useActionState` yields the initial state with `pending: false`, `useFormStatus` yields idle,
`useOptimistic` yields the base value. That is not a stub; it is the correct first render. A spinner
...
- `__jhFormMount()` — delegated submit interception over `[data-jh-form]`, called once by front 68's
  hydration entry, alongside front 27's `__jhLinkMount()`.

### Optimistic updates

`useOptimistic(base, apply)` returns the pair `#(value, push)`. Before hydration `value` is `base`
...
pub fn actionState(message: string) -> ActionState
...
**Acceptance:**
- [ ] `parseActionState("")` equals `actionState("")` with `ok: true` — no envelope means nothing has
      been submitted, not a failure
- [ ] The golden fixture from *Mechanism* decodes to `ok: false`,
      `message: "Title must be at least 3 characters"`, `redirectTo: ""`,
...
- [ ] A key that is neither `ok`, `message`, `redirect` nor `f.`-prefixed is ignored, not an error
...
```bp
pub type FormBinding(actionId: string, basePath: string, method: string)

pub fn formAction(actionId: string) -> FormBinding
pub fn withBasePath(binding: FormBinding, basePath: string) -> FormBinding
pub fn formAttrs(binding: FormBinding) -> Array<#(string, string)>
pub fn actionPath(binding: FormBinding) -> string
```

`withBasePath` returns a new record: there is no assignment to a `self` field, and the constructor
cannot take a default because declared defaults are never applied.

**Acceptance:**
- [ ] `actionPath(formAction("createPost")) == "/_onze13/action/createPost"`
- [ ] `actionPath(withBasePath(formAction("createPost"), "/app")) == "/app/_onze13/action/createPost"`
- [ ] `formAttrs` emits exactly four pairs, in the order `action`, `method`, `enctype`, `data-jh-form`
- [ ] `renderToString` of a form built with `formAttrs` contains `method="post"` — asserted, because
      an un-hydrated page depends on that one attribute
- [ ] An action id containing `/` or a space is rejected by `formAction`, naming the id

### Step 3 — `useActionState`

```bp
pub fn useActionState(
    actionId: string,
    initial: ActionState,
...
- [ ] After `__jhFormState` returns the golden fixture, `s.0.fieldError("title") == "Too short"`
...
- [ ] A component that calls `useActionState` twice with different ids gets two independent states

### Step 4 — `useFormStatus`

```bp
pub type FormStatus(pending: bool, actionId: string, method: string)

pub fn useFormStatus() -> @Context<Element, FormStatus>
```
...
**Acceptance:**
- [ ] `searchFormAttrs` emits `method="get"` and `action="/search"`, and `data-jh-form` is absent —
      a GET form is not an action submit and the submit interceptor must not claim it
- [ ] `data-jh-searchform="1"` marks it for the navigation interceptor instead
...
- [`examples/optimistic-like-example.bp`](./examples/optimistic-like-example.bp) — a like button that
  shows the new count before the server answers, and a nested submit button reading `useFormStatus`.
...
`useFormStatus` and `useOptimistic` are **not in this revision of `NEXTJS-DOCS.md`** — a grep for
...
have to invent. The rest of this front — the form binding, `useActionState`, the returned message,
...
|---|---|---|---|
| Tuple labels are lost through generic instantiation, so a hook's multi-value return is read positionally | `useActionState` (`s.0`/`s.1`/`s.2`) and `useOptimistic` (`o.0`/`o.1`) in every example | read by index and bind each element to a named `val` on the next line | preserve the written labels through instantiation (`hooks.bp:75-77` already files this) |
| No array or tuple destructuring in a binding — also recorded by front 01 | `val o = use useOptimistic(…); val shown = o.0;` where upstream writes `const [shown, push] = …` | one `val` per element, read by index | `val #(shown, push) = useOptimistic(…);` |
...
| A closure stored in a record field cannot be replaced after construction, so `push` must be handed out by the hook rather than rebound | `useOptimistic`'s `#(T, fn(action: T))` | the host cell holds the action list and the closure reads it | see the `self`-field gap — the same fix covers it |
...
`form_test.bp` is the attributes and the hooks' server-pass values: `formAttrs`'s four pairs and
their order, `actionPath` with and without a base path, the idle `FormStatus`, and
...
form must carry `action`, `method="post"` and `enctype`, and a test asserts the exact string. That is
the property that makes the un-hydrated path work, and it is falsifiable without a browser.
...
- [ ] The element constructors the examples call (`form`, `input`, `button`, `label`) are cited to the
      track-C front that lands them; this front adds none and `element.bp` is unmodified
- [ ] `useFormStatus` and `useOptimistic` are marked in `repository/jhonstart/docs.md` as specified
