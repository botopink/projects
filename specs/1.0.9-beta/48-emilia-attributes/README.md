...
> djb2-33 fold (seed 5381, masked to 32 bits) of the string `tokensToCss(tokens)`, and `tokens` is
> the element's token list in author order. Nothing else enters the hash. When a static class is
> also present, the `class` attribute value is `<static> + " " + <emilia class>` — static first, one
> ASCII space, no de-duplication, no sorting.**

Five consequences, each of which is a test:

1. **Pure function of the token list.** No counter, no module path, no request id, no build salt, no
   timestamp. The server and the client compute it from the same input and must get the same output.
   This is already true of `emilia()`; this front's contribution is to make it a stated contract
   with a test, so a later "optimization" that adds a per-render prefix fails visibly.
...
3. **ASCII only.** The JS cell folds UTF-16 code units and the erlang cell folds codepoints; they
   ...
   accepts only rule bodies that are ASCII, which every token in fronts 41–47 satisfies. The one
   way a non-ASCII body can arise is a payload leaf — `Color.Hex`, `Filter.Raw`, `Animate.Raw`,
   `Svg.Fill.Color` — carrying a non-ASCII string. `attributes.bp` is where that is checked.
...
```bp
pub fn cls(tokens: Token[]) -> string { … }        // className, re-exported under a short name
pub fn clsWith(base: string, tokens: Token[]) -> string { … }
```
...
pub fn styled(tokens: Token[]) -> #(string, string) {
    return #("class", emilia(tokens));
}

pub fn styledWith(base: string, tokens: Token[]) -> #(string, string) {
    return #("class", mergeClass(base, emilia(tokens)));
}
```
...
**Acceptance:**
- [ ] `styled(tokens)._0 == "class"` and `styled(tokens)._1 == emilia(tokens)` for a fixed list.
...
pub fn cls(tokens: Token[]) -> string {
    return className(tokens);
}

pub fn clsWith(base: string, tokens: Token[]) -> string {
    return mergeClass(base, className(tokens));
}
```
...
**Acceptance:**
- [ ] `val c = cls(tokens); val el = html """<div [class]={c}><p>hi</p></div>""";` renders
      `<div class="e_…"><p>hi</p></div>`.
- [ ] `cls(tokens) == className(tokens) == emilia(tokens)` for the same list — three names, one
      value, asserted so a refactor cannot split them.
...
- [ ] **Purity.** `className(cardTokens())` called twice in one test returns the same string, and
      called after an intervening `emilia()` of a different list still returns the same string — no
      counter leaks in.
- [ ] **Order is identity.** `className([.Text.Bold, .Color.Black]) != className([.Color.Black, .Text.Bold])`,
      asserted, so the contract is visible rather than incidental.
- [ ] **Merge order.** `styledWith("card", tokens)._1` starts with `"card "` and ends with the emilia
      class — asserted with `startsWith` and `endsWith`, in that order.
...
- [ ] **Cross-target agreement.** The class for `cardTokens()` is asserted against a **literal hex
      string**. That one line is the hydration gate: the `commonJS` row and the `erlang` row run the
      same assertion, and if the two hashes ever diverge the erlang row goes red. This assertion is
      what front 23 checks its SSR output against.
...
**What the coordinator must check against front 23.** The SSR pipeline writes the `class` attribute
into the HTML on the erlang side. It must use `styled` / `styledWith` / `classAttr` from this front
and must not spell `"class"` or join class names itself. The literal-hex assertion in
`integration_test.bp` is the shared fixture: front 23's SSR test asserts the same literal for the
same token list, and if the two literals ever differ, hydration is broken and one of the two tests
is red before a user sees it.
...
- [ ] The class-name contract — the five clauses — is written in `attributes.bp`'s docblock, not only
      in this README.
...
- [ ] The literal-hex fixture is shared with front 23 and both fronts assert it.
