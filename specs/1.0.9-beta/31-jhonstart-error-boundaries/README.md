...
**Wave:** 3
**Depends on:** 28 · 03 (content hash) · 17 (logging, read-only) · 63 (navigation signals, read-only)
**Owns:** `repository/jhonstart/src/error_boundary.bp`, `repository/jhonstart/test/error_boundary_test.bp`
**Does not touch:** `src/element.bp`, `src/hooks.bp`, `src/html.bp` (frozen), `src/router.bp` (26), `src/link.bp` (27), `src/server.bp` (28), `src/client.bp` (29), `src/suspense.bp`/`src/streaming.bp` (30)
...
| `global-error.bp` | `pub fn GlobalError(info: ErrorInfo) -> Element` | the root segment fails, or no other boundary caught | **yes** — it renders its own `html` and `body` |
...
`global-error.bp` needing `html` and `body` is the one place these conventions touch the element
surface: neither builder exists in `element.bp`, which is frozen. They come from the element-surface
front of track C (see *Language gaps*/*Blocked*, below).
...
1. A `[data-jh-error-boundary="ID"]` element is the catch target. A client component that throws
...
3. **`startTransition` errors are caught.** The same section names the exception. Front 68's runtime
   wraps a transition and routes a failure to the nearest `data-jh-error-boundary`, with a digest
   computed the same way.

### `reset` / `retry`

Upstream's `reset()` re-renders the failed subtree. jhonstart's fallback emits
`data-jh-error-reset="ID"`; front 29's `hydrate()` binds it, and the action it performs is front
...
pub fn wrap(id: string, tree: Element) -> Element {
    return Element(
        ...
        value: "",
        children: [tree],
        attrs: [#("data-jh-error-boundary", id)],
    );
}
...
- [ ] `global-error.bp`'s requirement to render its own `html`/`body` is stated, together with the
      fact that those builders do not exist yet and which front adds them
- [ ] `src/root.bp` declares `pub mod error_boundary;` and `botopink.json` lists it
...
`global-error.bp` cannot be written today. It must render its own `html` and `body`, and neither is
one of the eight builders in the frozen `element.bp`. This front does not add them; the element
surface is the element-surface front of track C (number allocated from 54 up), and `global-error.bp`
lands when that front does. The boundary record and the render path are complete without it, and
the example set does not pretend otherwise.
...
6. **`startTransition` errors are caught** — the boundary's wrapper carries
   `data-jh-error-boundary="ID"`, which is the anchor front 68's runtime routes a transition failure
...
- [ ] `error_boundary.bp` in the build tree, `root.bp` and `botopink.json` updated
...
- [ ] `global-error.bp` is listed under *Blocked* until the element-surface front lands
