...
**Wave:** 2
**Depends on:** 04 · 05
**Owns:** `src/context.bp`, `src/events.bp`, `src/lifecycle.bp` · `test/context_test.bp`, `test/events_test.bp`
...
1. **`src/rakun.d.bp` is not allocated to any front, and this front must change it.** Leaving the
   declaration-only `behavior Context` in place alongside a concrete `pub type Context` puts two
   `Context` declarations into every consumer's namespace, because `rakun.d.bp` is loaded for
   consumers through `botopink.json`'s `files` list. Front 06 needs to delete
   `rakun.d.bp:20-25` and update its docblock. Suggest adding `src/rakun.d.bp` to F06's row.
2. **`src/root.bp` needs `pub mod context; pub mod events; pub mod lifecycle;`** and is allocated to
   nobody. See front 05's *Contradictions* for the suggested fix.
3. **`botopink.json`'s `files` list** must gain the new declaration surface for consumers; it is also
   unallocated.
