...
> **Status (1.0.1-beta close):** closed classes: the **whole location class** (multi-line and `\\` token lines, interpolation hole spans, the tagged-call loc key, and the col-stored-as-byte-offset bug that put every parse error on line 1), the **4 legacy-syntax rows** (spec 05 items 5.12/5.13 - `@[` is now rejected with its own diagnostic and the fixtures use `#[@External.<Target>(...)]` and camelCase), and both vacuous `errors.zig` tests (the helper now fails when no `parseError` is filled). What is left is test-quality. Residuals in [`1.0.2-beta/09-review-tooling/README.md`](../../1.0.2-beta/09-review-tooling/README.md). The tables below are the audit record and are kept verbatim.
