# Front 04 — Tooling and docs

**Priority:** high — after F2 the editor still suggests and highlights `record`, `enum` and
`interface`, and the user docs teach a surface that no longer parses.
**Depends on:** F2 (final grammar; shares `language-server/src/engine.zig`) · F1 (shares
...
**Does not touch:** compiler-core, library repositories (F3), historical `specs/` and `tasks/`
...
- **Keyword token arrays**: `engine.zig:450,625,691,1040,1734,2187,3586` (compile-level changes land in F2; this front owns what they offer to the user).
