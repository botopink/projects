...
**Reference:** `repository/botopink-lang/docs/botopink-json.md` (the manifest) · `repository/rakun/modules/README.md` · `repository/botopink-lang/modules/compiler-cli/src/cli/{config,libs}.zig` (what a manifest means to the compiler) · `modules/lib-test-runner/src/discovery.zig` (what the gate can see) · `libs/std/AGENTS.md` § Tests · Spring Boot 4 starters (`/home/ericfillipe/develop/spring-boot-4/docs/02-desenvolvendo-com-spring-boot.md` § Starters) · Next.js project structure (`NEXTJS-DOCS.md` § 3) · Tailwind's package cut (`TAILWIND_CSS_DOCS.md` § 2: one core, three integration packages)

---

## Problem

Every library is a set of packages a consumer takes separately — a core, a test-helper package,
domain packages, runnable examples — and the compiler, the language server, the test runner and
`bpmp` must all see the same set by the same names. This front states that shape once, as the
workspace rule of decisions 75 and 76, and hands each library its `modules.md` to refine the cut.

## State

The manifest model (step 1) holds in every tool. jhonstart, emilia, rakun and erika are workspaces
with their core, `-test` member and example members (the member list per library is in
[`95-ecosystem-package-restructure/`](./95-ecosystem-package-restructure/README.md) § *Current
state*); `repository/onze` still holds the retired mocking library until the takeover (`../01-std`
step 5). Open: step 2's `rakun-web` / `files` / `path` box, the onze `-test` member and the moved
suites; step 3's example manifests, READMEs and `git` dependencies; step 4's helpers; the
formatter and merge boxes of the gate.

## Mechanism

The workspace rule of decisions [75](../decisions-taken.md#75-a-workspaces-manifest-npm-style-declares-a-librarys-members)
and [76](../decisions-taken.md#76-dependencies-is-the-object-form-only). `repository/botopink-lang/docs/botopink-json.md` is the authority for every field
...
import {Token, Theme, defaultTheme, styleRule} from "emilia";
...
// No re-implementation of std: a `-test` that defines its own `equals` is a bug.
```
...
- **A `-test` member re-exports nothing from std.** A consumer writes `import {testing.asserts} from "std"`
  beside `import { assertSheet } from "emilia-test"`.
- **Mocking is `testing.mocks`** (decision 71); a `-test` member keeps only the injection pairing.
...
- **An example depends on its own library's members with `{ "workspace": true }` and on other
  ...
  **outside** the ecosystem, after a release; `out/` is build output and is not committed.
...
A child of `examples/` without a manifest (the old `onze`'s loose `examples/mock_synthesis.bp`,
`jhonstart/examples/jhonstart-app/`) is not a member: the glob skips it, silently by design — a
directory that wants to be tested declares itself.
...
| An example may depend on **several** libraries — its own by `{ "workspace": true }`, others' cores or `-test`s by `path` | `emilia-card` → `jhonstart` + `emilia`; `onze-blog` → all four |
...
The order is fixed by [`../fronts.md`](../fronts.md) § Conflict rules: `01-std` retires
`repository/onze/` (the mocking library — its predicates are in `testing.asserts`, its mocks in
`testing.mocks`, decision 71; the repository is tagged and archived, decision 79); then this front
creates `repository/onze/` as the orchestrator workspace with the skeleton members (`06-onze/modules.md`
names them); then front 49 fills `modules/onze/src/`. The two never hold the directory at once, and
`grep -rl onze13 repository` is empty — as is every `**Owns:**` line and every manifest `name` under
`specs/1.0.10-beta/`; front 95 (beside this file) and `../unification.md` still carry the old name.
...
1. **Umbrella → workspace.** `repository/<lib>/botopink.json` loses `src`, `entry`, `files`,
   `dependencies` (each is a located error there) and gains `"workspaces": ["modules/*", "examples/*"]`;
...
2. **Core → `modules/<lib>/`.** The existing `src/*.bp` and `test/*.bp` move under
   ...
   consumer-visible module (`root.bp` first). `from "<lib>"` resolves to this member.
3. **Submodules → `{ "workspace": true }`.** Every `modules/<lib>-*/botopink.json` depends on its
   core with `{ "<lib>": { "workspace": true } }` (a `{ "path": "../../" }` is the *points at the
   workspace itself* refusal) and lists `files` — without them its rows read `✗ ships nothing`.
...
jhonstart, emilia, rakun and erika have made the four moves; onze makes them with the takeover.
A `-test` member is created empty (`root.bp` with an empty `pub` surface and one inline `test`) and
filled by its library's fronts on `testing.asserts` and `testing.snapshots`.
...
| **std** | — (bundled, not a workspace; `libs/std/src/` with `pub mod` in `root.bp`) | — (`testing.asserts` and `testing.snapshots` **are** the test surface every `-test` stands on) | — | `01-std/` |

`rakun-validation` is not a rakun member: it is the bundled library `validation` (decision 116,
`01-std/06-validation-lib`).
...
### Step 1 — The manifest model: workspaces, members, the object form

The `00 · 10-cli-residuals` carve-out decision 75 names: `modules/manifest/` (workspace vs package,
`workspaces` globs, members named by their manifests, `targets` inheritance), object-form
`dependencies` only (`git`/`path`/`{ "workspace": true }`, one source, one pin `branch|tag|rev`),
`manifest.scanRoots` in the runner, the loader, the LSP and `bpmp`, `botopink test` per member with
`✗ ships nothing`, `docs/botopink-json.md`, and the located errors of § 8.
...
The four moves of § 11, per library, each in its own worktree; onze after `01-std` has retired the
old `onze`.

**Acceptance:**
- [x] `import { Element } from "jhonstart";`, `import { Token } from "emilia";`, `import { … } from "rakun";` resolve to `modules/<lib>/` from a consumer under the roots — each pinned by an example member that imports it (`jhonstart-counter`/`-markup`/`-todo`, the fifteen `emilia-*` examples, `rakun-example`)
...
- [x] `zig build test-libs` lists every `modules/*` and `examples/*` member on every target its (inherited or restricted) `targets` allows, and no `rakun`/`jhonstart`/`emilia`/`onze` umbrella row — the manifests without a row are test fixtures (`jhonstart/repro/*`) and the old onze's `onze-demo`, none a member; onze's workspace rows arrive with front 95 step 2
- [x] `scripts/known-red-libs.txt` names every red row with a front number that exists in `specs/1.0.10-beta/`, and `zig build test-libs` exits 0 with them in place — no red row; the file holds only its header
- [x] every `AGENTS.md` in a moved directory reflects the tree, same commit
...
- [x] `git ls-files | grep 'examples/.*/out/'` is empty in every repository (jhonstart, emilia, rakun, erika, onze)
...
Stands on `testing.asserts` and `testing.snapshots`. Each `-test` member gains its first
...
- [x] `zig build test` green in `repository/botopink-lang` with `modules/manifest/` (step 1)
- [x] `zig build test-libs` green with the per-member row list as the acceptance artefact
- [ ] `botopink format --check` clean in every moved tree — **open:** the files front 95 moved (`jhonstart-link`, `rakun-app`) and the cores they left (`jhonstart`, `rakun`) would be reformatted — drift that predates the move (multi-line imports collapsed, braces dropped around one-statement `if` bodies); the new members are clean. `00 · 16-formatter`'s
- [x] `grep -rl onze13 repository` empty; no `**Owns:**` line, manifest `name` or directory under `specs/1.0.10-beta/` carries `onze13` (`../unification.md` and front 95 excepted); what remains names the rename
...
- The umbrella keeps a `botopink.json` because it is the workspace: the runner and the LSP find the
  library through it, and it answers no import.
- `path` is honoured by the loader and the LSP directly. Inside a workspace the sibling form is
  `{ "workspace": true }`, and a `path` to a sibling is a refusal — not a second spelling.
- The pin on a `git` dependency is `branch | tag | rev` (exactly one) — `bpmp install` needs the
  pin's kind, and a second spelling would be the knob decision 67 refuses (the *Implemented* note
  under 76).
- `01-std/asserts-api.md` is the authority for the assertion names.
