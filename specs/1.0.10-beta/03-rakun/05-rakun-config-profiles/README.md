...
- [x] An `Array<string>` field binds as a list, like `string[]`, and the catalogue shows its spelling; a
      list of another element type is refused at the annotation — held: `test/typed_config_test.bp` "rakun config: an Array<string> field binds as a list, like string[]" (the refusal: `src/config.bp` `configurationProperties` → `decl.fail("… cannot bind the list field …")`)
...
|---|---|---|---|
| `@Decl.Field` carries no default: a field's declared default applies at a call site, but a decorator reading the record cannot see `enabled: bool = false`. A `#[configurationProperties]` record therefore cannot carry its defaults where a reader would look for them. | `examples/typed-config-example.bp` — every default is written in the decorator's emitted reader (`rkPropBool(key, false)`) instead of on the field | Pass the default as the second argument of the typed reader, and declare it in `#[default("…")]` on the field so the catalogue can report it | `@Decl.Field.defaultValue`, the declared default's source text. Then `enabled: bool = false` is both the default and the documentation |
