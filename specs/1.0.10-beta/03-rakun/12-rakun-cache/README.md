...
### Package, module and import spellings

Downstream fronts import this module by name, so the spellings are a contract rather than a detail.
Front 24 already imports it in its example.

| Thing | Spelling |
|---|---|
| Package | `rakun-cache` — equal to the `modules/` directory name |
| Module holding the entry points | `cache`, at `modules/rakun-cache/src/cache.bp` |
...
only the other is a build break rather than a review comment.

### Test seams

This front keeps its state in the host, where a test cannot read it. Following the milestone
convention that front 19 states — *a front that mutates shared state exposes a read-only accessor for
tests* — it exposes three:

```bp
pub fn revalidatedPaths() -> Array<string>
pub fn revalidatedTags() -> Array<string>
pub fn clearRevalidated() -> i32
```

`revalidatedPaths()` is what lets a mutation test assert *what a server action invalidated* without
knowing how the cache stores tags. It returns a value, never a handle, and it clears nothing —
`clearRevalidated()` is separate and explicit, so a test that forgets it fails loudly instead of
inheriting the previous test's list.
...
| `test/revalidate_test.bp` | The six legality cells, `updateTag` read-your-own-writes, `revalidateTag` stale-then-fresh, multi-tag invalidation, the `revalidatedPaths()`/`revalidatedTags()` seams |
| `test/import_test.bp` | The qualified and bare import spellings both resolve |
...
- `revalidatedPaths()` exists and front 24's mutation test uses it.
