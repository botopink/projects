...
**Replaces:** `1.0.6-beta/06-security-auth`
...
4. Check `exp`, `nbf` and `iat` against `clock.now()` with a configurable skew (`rakun.security.jwt.clock-skew`,
...
## Carried from 1.0.6-beta F06 security-auth

Source: `specs/1.0.6-beta/06-security-auth/README.md`. Items below are present there and absent from the text above.

### 1. Spring reference names in the old Mechanism list
...
- `@PreAuthorize` — superseded by: *Method security, and why it is a proxy* (not ported; comptime failure naming supported forms).
- `@PostAuthorize`, `@EnableWebSecurity`, `JwtDecoder` — not named anywhere in 1.0.9 rakun fronts (no analogue, no decision recorded).

### 2. Step 1 — `SecurityContext`, `credentials`, `SecurityContextRepository`
...
- superseded by: *One filter, one policy, one context* — `Authentication` carries a `Principal` and no `credentials` field; the context lives in front 06's `Request` scope, so there is no load/save repository behavior.

### 3. Step 2 — builder-DSL policy and the bare `SecurityFilter`
...
- superseded by: *The policy* (`SecurityPolicy`/`PathRule` as a `#[provides]`-ed value, front 65's matcher, no `permitAll` default) and *What failure looks like* (RFC 9457 problem detail instead of a bare `"Unauthorized"` body). Order −300 is kept.

### 4. Step 3 — JWT: the secret key, `verifyJwt`, `JwtClaims`

| Old item | 1.0.9 status |
|---|---|
| `#[value("rakun.security.jwt.secret")] secret: string` | key **not named** above — the README lists `rakun.security.jwt.clock-skew` and `rakun.security.jwt.authorities-claim` but never the secret's key |
| `JwtAuthenticationFilter` type | no type name given for the JWT arm |
| `JwtClaims` type; claims `sub`, `roles`, `exp` | `exp`/`nbf`/`iat`/`iss`/`aud` and the `roles` claim are named; no `JwtClaims` type |
| `verifyJwt(token, secret) -> @Result<JwtClaims, string>` free function | no named verification entry point |
| `base64Decode`, `hmacSha256`, `parseJwtClaims` helpers | superseded by: *JWT* — front 01's `hmac` / `encoding` (constant-time `hmac.equals`) |
...
### 5. Step 4 — `BasicAuthenticationFilter`, bcrypt
...
- superseded by: *Basic authentication and passwords* — PBKDF2-HMAC-SHA256 is the only encoder; `bcrypt` fails at boot (NIF). No type name is given for the Basic arm in 1.0.9.

### 6. Step 5 — `UserDetailsService` names and the two arms

| Old spelling | 1.0.9 spelling |
|---|---|
| `fn loadUserByUsername(self, username) -> ?UserDetails` | `fn loadByUsername(self, username) -> ?UserDetails` |
| `UserDetails.password: string  // hashed` | `UserDetails.passwordHash: string` |
| `InMemoryUserDetailsService(#[value("rakun.security.users")] usersConfig: string)` | unnamed "in-memory arm" |
| `JdbcUserDetailsService(template: SqlTemplate)` | unnamed "SQL arm" over front 08's `#[query]` |

The user-list format is absent from 1.0.9 ("parses the configured user list", format unstated):

```bp
// parse usersConfig: "user1:pass1:ROLE_USER,user2:pass2:ROLE_ADMIN"
```

The SQL arm's statement is absent:
...
### 7. Step 6 — runtime check names and the JS runtime

| Old | 1.0.9 |
|---|---|
| `rkCheckAuthority("ROLE")` | `rkRequireAuthority("ROLE_ADMIN")` |
| `#[preAuthorize("hasRole('ADMIN') or #userId == authentication.principal")]` | superseded by: *Method security, and why it is a proxy* — not ported |
| "Works on both targets" | superseded by: `Target: erlang (server)` |
...
- superseded by: erlang-only target; no `.mjs` runtime half in 1.0.9.

### 8. Step 7 — CSRF: stateless header-rotation variant
...
- superseded by: *CSRF* — double-submit cookie + `X-CSRF-Token` header (note the casing change from `X-CSRF-TOKEN`), `PATCH` added, bearer-only requests exempt. Per-response token rotation via a response header is not in 1.0.9.

### 9. Step 8 — proposed source layout
...
- 1.0.9 names the test files (`policy_test.bp`, `jwt_test.bp`, `basic_test.bp`, `password_test.bp`, `method_security_test.bp`, `csrf_test.bp`) but no `src/` file layout.

### 10. Gate and blast radius rows

> - [ ] `botopink test --target commonJS` green
> - **Dependencies** — `jose` (JWT), `bcrypt` (password hashing)

- superseded by: `Target: erlang (server)`; *Current state* (no NIF; front 01's `hmac`/`encoding`, OTP `crypto`).
