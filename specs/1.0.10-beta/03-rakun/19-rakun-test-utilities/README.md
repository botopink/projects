...
**Acceptance:**
- [ ] `FakeRequest` implements `Request` and is accepted by a handler declaring a `Request` parameter, with no cast.
- [ ] Every accessor returns `""` for an absent name, never an optional.
- [ ] `header("Content-Type")` and `header("content-type")` return the same value.
- [ ] `withQuery`/`withHeader`/`withCookie`/`withParam` each return a new value, leaving the receiver unchanged.
- [ ] `fakeGet("/x").method` is `HttpMethod.Get` and `fakePost("/x", "b").body()` is `"b"`.
- [ ] Front 25's inline `FakeRequest` is deleted and its example imports this one — the duplicate does not survive the milestone.
...
**Acceptance:**
- [ ] `perform` on a registered route returns the handler's `Response`.
- [ ] `perform` on an unregistered path returns 404 from the router, not from the helper.
- [ ] A path parameter reaches the handler: `perform(fakeGet("/api/users/7"))` gives `req.param("id") == "7"` for a route registered as `/api/users/:id`.
- [ ] Query and header values from the double reach the handler through the real dispatch path.
- [ ] No socket is opened — asserted by running the whole suite with no port bound.

### Step 4 — Context control

**Acceptance:**
- [ ] `resetSingletons()` makes the next resolution construct a fresh instance; `rkBuildCount` goes back to zero.
- [ ] `resetContext()` empties the scan registry, the route table, the listener registry and the task registry, and a dispatch afterwards returns 404.
- [ ] Two `test` blocks in one file do not share a singleton when the second calls `resetSingletons()` first.
- [ ] `contextSnapshot()` reports scanned names, route paths and listener destinations, and is stable across runs.
...
**Acceptance:**
- [ ] The README and `AGENTS.md` show the `#[mock]` + `#[bean]` pairing as the `@MockBean` equivalent, and `modules/rakun-test/src/` contains no mocking implementation of its own.
- [ ] A test proves the pairing: a mocked `behavior` is the injected implementation of a controller's dependency, and `verify` sees the handler's call.
