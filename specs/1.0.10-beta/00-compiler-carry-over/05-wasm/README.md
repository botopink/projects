...
|---|---|---|---|
| **Self-recursion has no tail calls.** `fn count(n: i32, acc: i32) -> i32 { if (n == 0) { return acc; } else { return count(n - 1, acc + 1); } }` | `count(10000, 0)` answers `10000`; `count(100000, 0)` traps `wasm trap: call stack exhausted`, exit 134 | `100000` and `1000000` on both | **Closed** — `return f(args)` inside `fn f` re-binds the parameters and `br`s to a `(loop $__tail …)` wrapping the body; `run/tail_self_call.bp` green on four targets. Only the explicit `return f(…)` spelling is recognised (see `src/codegen/wat/AGENTS.md`) |
...
|---|---|---|---|
| **05** [`wasm`](./README.md) | `modules/compiler-core/src/codegen/wat.zig` · `modules/compiler-core/src/codegen/wat/**` · the `KNOWN` notes and new fixtures of its rows in `src/codegen/tests/**` | `modules/compiler-core/snapshots/codegen/wasm/**` (314) | steps 1–8; 4 of the 54 `expected-failures.txt` lines |
...
3. **05 × [`13-module-identity`](../13-module-identity/README.md) — `no` in substance, `yes` on paper.** 13 does not own `wat.zig`, but
   **step 2's box is the mechanism 13's named-type identity needs on this backend.** Do not merely
   ...
   [`../decisions-pending.md` #5](../../../1.0.5-beta/decisions-pending.md) asks on the commonJS side. **Recommended order:
...
6. **05 × [`10-cli-residuals`](../../../1.0.5-beta/10-cli-residuals/README.md) — `yes`.** No shared file. One row worth its attention: the
...
| [`05-wasm/`](./README.md) | high | Decision 8 at run time on wasm — the §7 formatter (a record prints as a raw heap address today), the box that carries `unknown` and unions, `is`, `case` arms and `loop` — plus a `Dict` lookup that answers the fallback with exit 0 and no diagnostic, `break <value>` returning a one-element array, `==` on tuples comparing references, and two string primitives that trap |
