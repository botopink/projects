...
| F2 | **An effect's annotation and its wrapper disagree** — `#[@asyncGenerator]` requires the wrapper `AsyncIterator` (`EffectKind.returnWrapper`), and no `AsyncGenerator` type exists | `builtins.d.bp:123`, `ast.zig:2072` |
...
F1 (rename one `Context`; the Expr-template record is the one with the narrower audience), F2 (one
vocabulary for the async-generator effect, annotation and wrapper), F4 (`Ok`/`Error` everywhere, or
...
`EffectKind.returnWrapper` value names a type declared in the file · the `Result::` spellings match
