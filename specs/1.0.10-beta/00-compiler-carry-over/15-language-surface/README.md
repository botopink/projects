> Carried from `specs/1.0.5-beta/15-language-surface/README.md`, status at carry (2026-09-20): landed (`109f6c97`); the hold-backs are C-05 (module-level `var`), C-11 (the trailing lambda's one-line body) and C-13 (decision 29's parser half, re-ordered by decision 60 — the patch is beside this file)
...
and filed none; [decision 14](../../../1.0.5-beta/decisions-taken.md#14-seven-forms-that-do-not-parse) settled them and
...
| `6907119` | **R5** — [decision 30](../../../1.0.5-beta/decisions-taken.md): `xs[0]`, `d["k"]`, `s[0]`, `xs[0..2]`, `xs[0..]`, `rows(1)[0]` |
| `fb230e5` | **R8** — [decision 28](../../../1.0.5-beta/decisions-taken.md): `a ?? 0` |
| `9daf390` | **R7** — [decision 33](../../../1.0.5-beta/decisions-taken.md): a bodyless `fn` declares its return type, and says so when it does not |
...
- **[Decision 29](../../../1.0.5-beta/decisions-taken.md#29-does-a-block-shaped-statement-end-itself) — the parser half
...
- **[Decision 36](../../../1.0.5-beta/decisions-taken.md#20-is-a-pattern-range-inclusive) — untouched**, because
...
`??` is a form the language does not want — [decision 14](../../../1.0.5-beta/decisions-taken.md#14-seven-forms-that-do-not-parse)
...
- [ ] The assignment is recorded in [`../decisions-pending.md`](../../../1.0.5-beta/decisions-pending.md) as a new
...
a question in [`../decisions-pending.md`](../../../1.0.5-beta/decisions-pending.md) with a recommendation; a form
...
- [ ] Every row names an owner — this front, another front, or "unowned", and every unowned row is
      opened in [`../decisions-pending.md`](../../../1.0.5-beta/decisions-pending.md) in the same commit
...
- [ ] A `reject/` cell per form, handed to [`12-language-tests`](../12-language-tests/README.md) with
      ...
      does not exist ([decision 15](../../../1.0.5-beta/decisions-taken.md#15-a-lower-case-externalnode-)'s lower-case
...
| E | **Is `Option` a value-level name?** `decision-8:66`, `:84` and `docs.md:396` write `Option.None` and `Some(1)`; neither is in scope, although `?T` and `.unwrapOr()` work | unowned | **the documents are wrong** — [decision 2](../../../1.0.5-beta/decisions-taken.md#2-optiont-does-not-exist-either) already settled that `?T` is the only spelling of the type; the value-level names follow it |
