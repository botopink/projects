...
`grep -n 'arityMismatch' modules/compiler-core/src/comptime/infer.zig` gives **twelve**
raise sites. They fall into three classes:

| Class | Sites (`infer.zig`) | Accounts for defaults |
|---|---|---|
...
type the call as a fresh var. Since C9 that fallback reds when the receiver's type is
