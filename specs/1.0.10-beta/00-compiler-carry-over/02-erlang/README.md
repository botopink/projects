...
- **`status.md` § Pending** carries further erlang rows, each with its repro: a bare `return;` inside
  an `if` of a `@Task<void>` body does not leave the function; `patternNodeExtra` asks
  ...
  the runner's `__bp_load_siblings/0` skips a module `erlc` refuses silently.
