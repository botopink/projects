...
- **A page signal is jhonstart's end to end** (decision 117). jhonstart's render writes to a generic
  ...
  are `fn … -> @Component<Element>` (decisions 117 and 118).
- **The return type is the annotation** (decisions 118–127). A function gains `throw` / `try`,
  `await`, `use` or `yield` by writing `@Result`, `@Task`, `@Use` / `@Component`, `@Iterator` or
  `@Stream` as its return; only `@Result` fails; there are no effect annotations and no
  compatibility window. The library fronts' examples were written before these decisions and spell
  the old annotations until `00 · 24-effects-by-return` rewrites them with its codemod (step E8).
