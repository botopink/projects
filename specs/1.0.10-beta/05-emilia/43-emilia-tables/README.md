...
## Carried from 1.0.8-beta F09 emilia-tables
...
**The `16` step of the spacing scale** — complements Step 2; missing because the 1.0.10 scale was
pinned to the `Pad`/`Margin` step set `{0, 1, 2, 4, 8}`. The 1.0.8 draft declared one more step on
all three axes:

```bp
Table {
    BorderSpacing  { 0, 1, 2, 4, 8, 16 }
    BorderSpacingX { 0, 1, 2, 4, 8, 16 }
    BorderSpacingY { 0, 1, 2, 4, 8, 16 }
}
```

Under contract `§ 4a` the missing rows would read `.Table.Spacing.16` → `border-spacing:calc(var(--spacing) * 16)`,
`.Table.SpacingX.16` → `border-spacing:calc(var(--spacing) * 16) 0`, `.Table.SpacingY.16` →
`border-spacing:0 calc(var(--spacing) * 16)`. Whether the step joins the scale is a decision for
this front; today the reference-derived scale above stops at `8`.

**The 1.0.8 section and leaf spellings** — complements Step 1 and Step 3's naming rationale;
missing because 1.0.10 renamed them. The old draft spelled the sub-sections
`BorderCollapse { Collapse, Separate }`, `BorderSpacing`, `BorderSpacingX`, `BorderSpacingY`,
`Layout { Auto, Fixed }`, `CaptionSide { Top, Bottom }`, and wrote numeric leaves as `__N` in
expression position (`.Table.BorderSpacing.__4`). The 1.0.10 spellings are `.Table.Collapse` /
`.Table.Separate` (bare leaves), `.Table.Spacing.N` / `.SpacingX.N` / `.SpacingY.N`,
`.Table.Layout.*`, `.Table.Caption.*`, with numeric leaves written as bare digits in expression
position and matched as `__N` only inside a `case` pattern.

**The 1.0.8 acceptance list, with resolved values** — complements Step 2's acceptance; missing
because contract `§ 4a` forbids a `rem` literal in the dispatcher. The old bullets are the values
the 1.0.10 rows resolve to under `defaultTheme()` (`--spacing: 0.25rem`):

- [ ] `Table.BorderCollapse.Collapse` → `border-collapse:collapse`
- [ ] `Table.BorderSpacing.__4` → `border-spacing:1rem` — **superseded**: `.Table.Spacing.4` emits `border-spacing:calc(var(--spacing) * 4)`, which resolves to `1rem`
- [ ] `Table.BorderSpacingX.__2` → `border-spacing:0.5rem 0` — **superseded**: `.Table.SpacingX.2` emits `border-spacing:calc(var(--spacing) * 2) 0`, which resolves to `0.5rem 0`
- [ ] `Table.Layout.Fixed` → `table-layout:fixed`
- [ ] `Table.CaptionSide.Bottom` → `caption-side:bottom`

**Dependency as first drafted** — complements the header; missing because 1.0.10 depends on 35
(spacing) instead. The 1.0.8 draft read `Depends on: F15 (modifiers)`, i.e. what is now front 34.

**Gate** — complements *Test plan* and *Definition of done*; both bullets are covered above but the
old wording is kept:
...
- `tokens.bp`: +~30 lines
- `emilia.bp`: +~40 lines
- `test/tables_test.bp`: new file, ~10 tests
...
under these names: `"border-collapse generates correct CSS"`, `"border-separate generates correct CSS"`,
`"border-spacing-4 generates correct CSS"` (asserted `border-spacing:1rem`),
`"border-spacing-x-2 generates correct CSS"` (`border-spacing:0.5rem 0`),
`"border-spacing-y-2 generates correct CSS"` (`border-spacing:0 0.5rem`),
`"table-layout-fixed generates correct CSS"`, `"table-layout-auto generates correct CSS"`,
`"caption-side-top generates correct CSS"`, `"caption-side-bottom generates correct CSS"`.

Examples carried:
- [`./examples/tables-example-1.0.8.bp`](./examples/tables-example-1.0.8.bp) — the 1.0.8 file, kept beside the 1.0.10 rewrite because the two differ (`cmp`); it uses the 1.0.8 spellings (`.Table.BorderCollapse.Collapse`, `.Table.BorderSpacing.__4`, `.Table.CaptionSide.Top`) and resolved `rem` values, and is reference material, not a build target.
