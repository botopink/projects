...
## Carried from 1.0.8-beta F15 emilia-modifiers

The 1.0.8 draft built each modifier as a string-concatenation arm (`":hover{" + … + "}"`) with
pre-v4.3 selectors and `px` breakpoints; the sections above replace that with front 56's
two-field `Variant` and the v4.3 `rem` / `&`-prefixed forms. Where the two disagree the sections
above win. The items below are what the 1.0.8 draft carried that does not appear above.

### 1.0.8 § Step 4 — three group/peer variants not in the 1.0.10 table

Complements § Token surface › *Parent and sibling state*, which lists five `Group*` and six
`Peer*` variants. The 1.0.8 draft named three more (and lacked `GroupDisabled`, `GroupOpen`). The
1.0.8 selectors are the pre-v4.3 `.group:hover &` / `.peer:focus ~ &` forms; under the template
above they would read `&:is(:where(.group):visited *)`, `&:is(:where(.peer):active ~ *)` and
...
### 1.0.8 § Step 8 — container query modifiers

Absent above by decision: § Reference gaps lists container queries as out of scope ("a separate
wrapper shape (`@container (width >= …)`) and deserves its own front"). The 1.0.8 draft declared
five of them with `min-width` queries; recorded here so the future front starts from the same
list.
...
1.0.8 acceptance for it: `ContainerMd([Token.FlexRow])` → `@container(min-width:28rem){flex-direction:row}`.
1.0.8 note: container query modifiers use `@container` instead of `@media`.

### 1.0.8 § Step 1 / § Step 9 — the pre-v4.3 emitted forms

Complements § Token surface › *Breakpoints* and § Step 1's "today → after" table, which record the
`768px`/`1024px`/`1280px` queries the six existing modifiers emit today but not the `px` values
the 1.0.8 draft assigned to the new ones. All of these are superseded by the `rem` and `&` forms
above (§ Step 2: "no breakpoint emits a `px` value").
...
1.0.8 also spelled `Open` as `&[open]{...}` (above: `&:open, &:popover-open`), the pseudo-classes
without a leading `&` (`:visited{...}`, `:first-child{...}`), the pseudo-elements as
`&::placeholder`, `&::before`, `&::after`, `&::selection`, `& ::marker`, `&::first-line`,
`&::first-letter`, `&::file-selector-button`, `&::backdrop`, and the media forms without spaces
(`@media(prefers-reduced-motion:reduce)`, `@media(orientation:portrait)`,
`@media(prefers-contrast:more)`, `@media(forced-colors:active)`).

1.0.8 § Step 9 acceptance, verbatim (hex colours and `rem` lengths are the 1.0.8 spacing/colour
forms; the modifier wrapper is the part this front owns):
...
- [ ] Modifiers nest: `Md([Hover([Token.BgBlack])])` → `@media(min-width:768px){:hover{background-color:#000000}}`

### 1.0.8 § Blast radius

Absent above — the 1.0.9/1.0.10 template has no blast-radius section.

- `tokens.bp`: +~80 lines (new modifier variants)
- `emilia.bp`: +~100 lines (arms in the `tokenToCss` switch)
- `test/modifiers_test.bp`: new file, ~40 tests
- Every other front may use the new modifiers

### 1.0.8 § Notes

Three of the four are covered above (`.group`/`.peer` are the consumer's classes — § Step 6; dark
mode is `prefers-color-scheme` with `@custom-variant` out of scope — § Step 3, § Reference gaps;
free nesting — § Step 7). Carried verbatim:

- Group/Peer modifiers require the parent element to carry the `.group` or `.peer` class
- Container query modifiers use `@container` instead of `@media`
- Dark mode uses `prefers-color-scheme` by default; it can be customised via `@custom-variant`
- Modifiers nest freely: `Dark([Md([Hover([Token.BgBlack])])])`

### 1.0.8 example tests

The 1.0.8 example ends with 24 inline tests named `"<variant> modifier generates correct CSS"`
(`sm`, `2xl`, `dark`, `hover`, `first`, `last`, `odd`, `even`, `disabled`, `required`, `invalid`,
`group-hover`, `peer-focus`, `placeholder`, `before`, `print`, `motion-reduce`, `portrait`, `rtl`,
`ltr`, `open`, `container-md`) plus `"nested modifiers compose correctly"` and
`"triple nested modifiers compose correctly"` (the latter asserting
`@media(prefers-color-scheme:dark){@media(min-width:768px){:hover{background-color:#4f46e5}}}`).
Its token lists also reach into other fronts' sections with 1.0.8 spellings: `.Outline.__2`,
`.Outline.Offset.__2`, `.Border.Blue.__500`, `.Effect.Opacity.__50`, `.Interact.Cursor.NotAllowed`,
`.Animate.None`, `.Animate.Spin`, `.Transition.None`, `.A11y.ForcedColorAdjust.None`, and the
dot-shorthand modifier call `.Hover([.Bg.Blue.__50])` inside a `Token[]`.

Examples carried:
- [`./examples/modifiers-example-1.0.8.bp`](./examples/modifiers-example-1.0.8.bp) — the 1.0.8
  `modifiers-example.bp`; a same-named 1.0.9 file already exists and differs (`cmp`), so the 1.0.8
  one is kept beside it with the `-1.0.8` suffix
