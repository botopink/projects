...
## Carried from 1.0.8-beta F08 emilia-filters

The 1.0.8 draft (`1.0.8-beta/08-emilia-filters`, Portuguese) was compared section by section with
the front above. Every family and step of the 1.0.8 draft is present above — the backdrop section
and three `Full` leaves under a different name, listed first — and two details are not.

### Path renames — mapping only, nothing missing

Complements Step 1 and Step 3:

| 1.0.8 path | 1.0.10 path above |
|---|---|
| `BackdropFilter.*` (top-level section) | `.Backdrop.*` |
| `Filter.Grayscale.Full` / `Filter.Invert.Full` / `Filter.Sepia.Full` (`100%`) | `.Filter.Grayscale.100` / `.Filter.Invert.100` / `.Filter.Sepia.100` |
| `BackdropFilter.Grayscale.Full` / `Invert.Full` / `Sepia.Full` | `.Backdrop.Grayscale.100` / `.Backdrop.Invert.100` / `.Backdrop.Sepia.100` |
| `BackdropFilter.Opacity.{0, 5, 10, 25, 50, 75, 100}` (seven steps) | `.Backdrop.Opacity.*` — the fifteen steps of `§ 12.3`, a superset |

### The literal blur ladder

Complements Step 1's bullet "no `px` literal appears in any `Filter` arm, because the blur scale is
a theme entry": the front above emits `blur(var(--blur-*))` and names only `8px` in prose. The
1.0.8 draft pins every step, which are the values front 54's `--blur-*` entries need (the same
entries `§ 13.2`'s backdrop rows read):

| 1.0.8 token | Theme entry above | Value |
|---|---|---|
| `Filter.Blur.Xs` | `--blur-xs` | `4px` |
| `Filter.Blur.Sm` | `--blur-sm` | `8px` |
| `Filter.Blur.Md` | `--blur-md` | `12px` |
| `Filter.Blur.Lg` | `--blur-lg` | `16px` |
| `Filter.Blur.Xl` | `--blur-xl` | `24px` |
| `Filter.Blur.X2xl` | `--blur-2xl` | `40px` |
| `Filter.Blur.X3xl` | `--blur-3xl` | `64px` |

### 1.0.8 acceptance bullets in resolved, single-declaration form

Complements Step 1 and Step 3: the 1.0.8 draft pinned each filter as one resolved `filter:` /
`backdrop-filter:` declaration with a leading-zero decimal. Two things the front above settles
differently — `brightness(.5)` not `brightness(0.5)` (byte-equality with the reference), and the
two-declaration `--tw-<family>:…;filter:var(--tw-filter)` shape so two filters compose. Kept as the
function and value each bullet pins:
...
Blast radius: `tokens.bp` +~80 lines · `emilia.bp` +~120 lines · `test/filters_test.bp` new file,
~25 tests.

Examples carried:
- [`./examples/filters-example-1.0.8.bp`](./examples/filters-example-1.0.8.bp) — the 1.0.8 example
  (blur, brightness, contrast, drop-shadow, grayscale, hue-rotate, invert, saturate, sepia and the
  backdrop mirror); a same-named file already exists above and differs, so the older one is kept
  with the `-1.0.8` suffix.
