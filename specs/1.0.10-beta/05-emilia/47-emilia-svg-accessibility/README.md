...
## Carried from 1.0.8-beta F13 emilia-svg-accessibility
...
**The `sr-only` and `not-sr-only` bodies** — complements Step 3; missing because 1.0.10 gates the
two bodies on a dated read of the upstream page rather than transcribing them. The 1.0.8 draft
wrote them out as leaf comments and acceptance bullets. They are recorded here as the 1.0.8
draft's literal — the Step 3 gate still applies, and the upstream read either confirms these
strings or replaces them:
...
- [ ] `A11y.SrOnly` → `position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border-width:0` — nine declarations
- [ ] `A11y.NotSrOnly` → `position:static;width:auto;height:auto;padding:0;margin:0;overflow:visible;clip:auto;white-space:normal` — eight declarations; note it does **not** restore `border-width`, so Step 3's "restores every property `SrOnly` sets" bullet would fail against this literal as written and the upstream read decides

**The 1.0.8 palette-leaf colour sections** — complements Step 1 and *Reconciled with front 33*;
**superseded** by `Token.SvgFill(value)` / `Token.SvgStroke(value)`, kept so the rejected shape is
on record:
...
The old draft spelled the keyword `currentColor` (camel-case); 1.0.10 emits the CSS keyword
lowercase, `fill:currentcolor` / `stroke:currentcolor`. Both are valid CSS (the keyword is
case-insensitive); the test literal is the 1.0.10 spelling. `White`/`Black` go through the payload
with front 33's spelling.

**The 1.0.8 acceptance list** — complements Steps 1, 2 and 4; the hex bullets are **superseded**
by `paletteVar`, the rest are covered above:

- [ ] `Svg.Fill.None` → `fill:none`
- [ ] `Svg.Fill.Current` → `fill:currentColor` — 1.0.10 spells it `fill:currentcolor`
- [ ] `Svg.Fill.Red.__500` → `fill:#ef4444` — **superseded**: `Token.SvgFill(value: paletteVar("red", 500))` → `fill:var(--color-red-500)`; `#ef4444` is what the theme entry resolves to
- [ ] `Svg.Stroke.Current` → `stroke:currentColor` — 1.0.10 spells it `stroke:currentcolor`
- [ ] `Svg.Stroke.Blue.__500` → `stroke:#3b82f6` — **superseded**: `Token.SvgStroke(value: paletteVar("blue", 500))` → `stroke:var(--color-blue-500)`; `#3b82f6` is what the theme entry resolves to
...
`Depends on: F14 (colors)`, which is front 33 today — the same dependency, renumbered.
...
- `tokens.bp`: +~40 lines
- `emilia.bp`: +~60 lines
- `test/svg_a11y_test.bp`: new file, ~15 tests
...
under these names: `"svg-fill-none generates correct CSS"`, `"svg-fill-current generates correct CSS"`
(asserted `fill:currentColor`), `"svg-fill-red-500 generates correct CSS"` (`fill:#ef4444`),
`"svg-stroke-current generates correct CSS"` (`stroke:currentColor`), `"svg-stroke-blue-500 generates correct CSS"`
(`stroke:#3b82f6`), `"svg-stroke-width-2 generates correct CSS"`, `"sr-only generates correct CSS"`
(asserted `position:absolute`, `width:1px`, `height:1px`, `overflow:hidden` individually),
`"not-sr-only generates correct CSS"` (`position:static`, `width:auto`, `height:auto`),
`"forced-color-adjust-none generates correct CSS"`, `"forced-color-adjust-auto generates correct CSS"`.

Examples carried:
- [`./examples/svg-a11y-example-1.0.8.bp`](./examples/svg-a11y-example-1.0.8.bp) — the 1.0.8 file, kept beside the 1.0.10 rewrite because the two differ (`cmp`); it uses `.Svg.Fill.Red.__500`-style palette leaves, `currentColor` and hex assertions, and is reference material, not a build target.
