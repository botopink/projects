...
**Replaces:** `1.0.8-beta/06-emilia-borders`
...
## Carried from 1.0.8-beta F06 emilia-borders

The 1.0.8 draft (`1.0.8-beta/06-emilia-borders`, Portuguese) was compared section by section with
the front above. Every sub-section and leaf of the 1.0.8 draft is present above — the per-side and
per-corner radii under a different path, listed first — and two details are not.

### Path renames — mapping only, nothing missing

Complements *Border radius — `§ 11.1`, `§ 21.5`*:

| 1.0.8 path | 1.0.10 path above |
|---|---|
| `Border.RoundedT.*` / `RoundedR` / `RoundedB` / `RoundedL` | `.Border.Rounded.T.*` / `.R.*` / `.B.*` / `.L.*` |
| `Border.RoundedTl.*` / `RoundedTr` / `RoundedBr` / `RoundedBl` | `.Border.Rounded.Tl.*` / `.Tr.*` / `.Br.*` / `.Bl.*` |
| `Outline.Color.{Red, Blue, Gray, White, Black, Transparent, Hex(value)}` | `.Outline.Color.*` over front 33's full grid |

### The literal radius ladder

Complements Step 2's bullet "the eight `--radius-*` values of `§ 21.5` are in front 54's theme
layer": the front above emits `var(--radius-*)` and does not print the values. The 1.0.8 draft pins
them as comments on the leaves. Its ladder has no `Xs` and its `Sm` is the v3 value, so the
two must be reconciled against `§ 21.5` (which lists `--radius-xs` through `--radius-4xl`) before
the theme entries are written:

| 1.0.8 token | Value |
|---|---|
| `Border.Rounded.None` | `0` |
| `Border.Rounded.Sm` | `0.125rem` |
| `Border.Rounded.Md` | `0.375rem` |
| `Border.Rounded.Lg` | `0.5rem` |
| `Border.Rounded.Xl` | `0.75rem` |
| `Border.Rounded.X2xl` | `1rem` |
| `Border.Rounded.X3xl` | `1.5rem` |
| `Border.Rounded.X4xl` | `2rem` |
| `Border.Rounded.Full` | `9999px` |

### 1.0.8 acceptance bullets in resolved-literal form

Complements Step 1 and Step 2: four of the 1.0.8 bullets pin a resolved hex or `rem`, which the
front above replaces with `var(--color-*)` / `var(--radius-*)`. Kept as the literal they pin:
...
The remaining 1.0.8 bullets appear above verbatim:
...
Blast radius: `tokens.bp` +~80 lines · `emilia.bp` +~150 lines · `test/borders_test.bp` new file,
~30 tests.

Examples carried:
- [`./examples/borders-example-1.0.8.bp`](./examples/borders-example-1.0.8.bp) — the 1.0.8 example
  (expanded radius, border-style, outline width/colour/style/offset); a same-named file already
