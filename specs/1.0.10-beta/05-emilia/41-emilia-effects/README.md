...
**Acceptance:**
- [ ] `effectTokenToCss(.Shadow.Md, th)` returns `box-shadow:var(--shadow-md)` — the string
      `box-shadow:md` appears nowhere in `repository/emilia/src/`.
- [ ] `Token.EffectShadowRaw(value: "0 0 0 1px red")` **constructs** — a test builds one, which is
      the check that would have failed against a nested `Effect.Shadow.Raw`.
- [ ] No section in this front's `tokens.bp` block contains a payload leaf.
- [ ] No shadow arm contains an `rgb(` literal: the scale's values live in the theme, and a resolved
      literal inside a dispatcher is the defect this acceptance exists to catch.
...
- [ ] Each of the twelve values is character-identical to its row above, which is character-identical to the "Propriedade CSS" column of `TAILWIND_CSS_DOCS.md § 12.1`.
...
**Acceptance:**
- [ ] `effectTokenToCss(.TextShadow.Sm, th)` returns `text-shadow:var(--text-shadow-sm)`.
- [ ] `.Effect.TextShadow.None` returns `text-shadow:none` — a keyword, not a theme lookup.
...
- [ ] `effectTokenToCss(.Opacity.60, th)` returns `opacity:0.6` — not `opacity:.6`.
...
- [ ] `blendTokenToCss(.Mix.PlusLighter, th)` returns `mix-blend-mode:plus-lighter`.
...
- [ ] A list of `[.Mask.Clip.Padding, .Mask.Mode.Luminance]` composes to the two declarations
      `mask-clip:padding-box` and `mask-mode:luminance` in `Rule.declarations`, in list order.
...
**Acceptance:**
- [ ] The five new arms sit between front 40's arm and front 42's, in that order.
- [ ] Each arm is one `declSheet(...)` call; no arm in this front builds a `Rule` or a `Sheet` by
      hand.
- [ ] The variant arms remain last in the `case`, and front 34 owns their wrapping — this front adds
      none.
- [ ] `tokenToSheet` still has no `_` arm — a token with no arm is a compile error, not a silent `""`.
...
- [ ] `effectTokenToCss`, `blendTokenToCss`, `maskTokenToCss` and their sub-dispatchers are fenced
      ...
      takes `th: Theme` per contract `§ 4a`.
- [ ] `box-shadow:sm` and its three siblings are gone from the repository.
- [ ] The shadow, inset-shadow and text-shadow scales are `themeVar(...)` lookups; no `rgb(` literal
      appears anywhere in this front's block.
...
## Carried from 1.0.8-beta F07 emilia-effects

The 1.0.8 draft (`1.0.8-beta/07-emilia-effects`, Portuguese) was compared section by section with
the front above. The blend sections are absorbed under a different path, listed first; the items
after that are leaves and values the 1.0.8 draft declares that the text above does not.

### Path renames — mapping only, nothing missing

Complements Step 1 and Step 4:

| 1.0.8 path | 1.0.10 path above |
|---|---|
| `Blend.*` (top-level, `mix-blend-mode`, seventeen leaves) | `.Blend.Mix.*` |
| `BgBlend.*` (top-level, `background-blend-mode`, seventeen leaves) | `.Blend.Bg.*` |
| `Effect.Shadow.2xs` (matched as `__2xs`) | `.Effect.Shadow.X2xs` |
| `Effect.InsetShadow.2xs` / `Effect.TextShadow.2xs` | `.Effect.InsetShadow.X2xs` / `.Effect.TextShadow.X2xs` |

### The shadow literals the 1.0.8 draft pins

Complements Step 1, Step 2 and the *Mechanism* paragraph "the literal each variable holds belongs
to the theme": the front above never prints a literal, by design, and points at `§ 21.4` for the
box-shadow scale and at nothing for the inset and text-shadow scales. The 1.0.8 draft pins these,
which are the values front 54's theme entries need:

| 1.0.8 token | Theme entry above | Value |
|---|---|---|
| `Effect.Shadow.2xs` | `--shadow-2xs` | `0 1px rgb(0 0 0 / 0.05)` |
| `Effect.Shadow.Xs` | `--shadow-xs` | `0 1px 2px 0 rgb(0 0 0 / 0.05)` |
| `Effect.Shadow.X2xl` | `--shadow-2xl` | `0 25px 50px -12px rgb(0 0 0 / 0.25)` |
| `Effect.Shadow.Inner` | — (literal above too) | `inset 0 2px 4px 0 rgb(0 0 0 / 0.05)` |
| `Effect.TextShadow.Sm` | `--text-shadow-sm` | `0px 1px 0px rgb(0 0 0 / 0.075),...` — the 1.0.8 draft itself truncates the list |

The 1.0.8 acceptance bullets that read these as resolved declarations are superseded by the
`var(--shadow-*)` form above and are kept as the value they pin:
...
### Six extra opacity steps

Complements Step 3: the table above has the fifteen steps `§ 12.3` prints. The 1.0.8 draft declared
twenty-one — the six not above are `15`, `35`, `45`, `55`, `65`, `85`, i.e. `.Effect.Opacity.15` →
`opacity:0.15`, `.35` → `opacity:0.35`, `.45` → `opacity:0.45`, `.55` → `opacity:0.55`, `.65` →
`opacity:0.65`, `.85` → `opacity:0.85`. Missing because the reference prints fifteen and the extra
six are Tailwind's bare-integer `opacity-<number>` form.
...
### Mask leaves beyond the twenty of `§ 12.6`

Complements Step 5: the table above carries `Mask.Position.Center` only, `Mask.Repeat.{NoRepeat, Repeat}`
only and `Mask.Size.{Cover, Contain}` only. The 1.0.8 draft declared the full keyword ladders.
Missing because `§ 12.6` lists twenty utilities and the front above declares exactly those.
...
Under the paths above: `.Mask.Position.Top` → `mask-position:top`, `.Bottom` → `mask-position:bottom`,
`.Left` → `mask-position:left`, `.Right` → `mask-position:right`; `.Mask.Repeat.RepeatX` →
`mask-repeat:repeat-x`, `.RepeatY` → `mask-repeat:repeat-y`, `.Round` → `mask-repeat:round`,
`.Space` → `mask-repeat:space`; `.Mask.Size.Auto` → `mask-size:auto`.

### 1.0.8 acceptance bullets covered above
...
Blast radius: `tokens.bp` +~120 lines · `emilia.bp` +~200 lines · `test/effects_test.bp` new file,
~35 tests.

Examples carried:
- [`./examples/effects-example-1.0.8.bp`](./examples/effects-example-1.0.8.bp) — the 1.0.8 example
  (expanded box-shadow, text-shadow, full opacity, mix/background blend, mask); a same-named file
  already exists above and differs, so the older one is kept with the `-1.0.8` suffix.
