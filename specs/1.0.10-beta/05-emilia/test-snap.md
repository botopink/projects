...
The snapshot suite to write for the landed modules. The fronts landed with inline string asserts in
the source files; this map specifies the `emilia-test` helpers and the per-front `test/` files that
pin the same output as `.snap` files, which no flag ever accepts. Each test below is followed by the
exact `.snap` file it produces, rendered from the modules as they ship. Paths are relative to
`repository/emilia/`.
...
`import { assertCss, … } from "emilia-test";`; `equal`/`contains` come from `testing.asserts` (`import { testing: { asserts: { equal, contains } } } from "std";` — decisions 106/107). A
...
Refusals (a `Variant` with two `&`, `extendTheme` with an unknown prefix, `arbValue` with `}`) are build
...
```bp
import { Token, paletteVar, paletteEntries, defaultTheme, extendTheme } from "emilia";
...
neutral, 50 → 950 inside each), transcribed from upstream `tailwindcss` 4.3.2 `theme.css`:
```
...
```
The file is written once from `paletteEntries()` and never regenerated by a flag.
...
test "css: layout ---- columns and breaks" {
    try assertCss(@src(), [.Layout.Columns.3, .Layout.Columns.X3xs, .Layout.Columns.X7xl, .Layout.BreakInside.AvoidColumn, .Layout.BreakAfter.Page]);
}
...
test "css: flex ---- alignment keeps the flex start asymmetry" {
    try assertCss(@src(), [.Flex.Justify.Start, .Flex.Justify.Between, .Flex.JustifyItems.Start, .Flex.Items.Baseline, .Flex.AlignSelf.Start, .Flex.Content.Evenly, .Flex.PlaceContent.Between]);
}
...
test "css: filters ---- backdrop blur and opacity" {
    try assertCss(@src(), [.BackdropFilter.Blur.None, .BackdropFilter.Blur.Sm, .BackdropFilter.Opacity.50]);
}
...
```
The two bodies are upstream `theme.css`'s; the front gates them on an upstream
...
## 48 — attributes · `modules/emilia/test/attributes_test.bp`

```bp
// attributes_test.bp
import { Token, styled, styledWith, className, cls, mergeClass, assertAsciiBody, fullTheme, emiliaWith, extendTheme, ThemeEntry, flush } from "emilia";
import { assertClassName } from "emilia-test";
import { testing: { asserts: { equal, truthy, falsy, contains } } } from "std";
...
```bp
// attributes_test.bp, continued — emilia only; no HTML is rendered
...
test "the slot appends after the attributes before it" {
    val th = fullTheme();
    val attrs: Array<#(string, string)> = [#("id", "x"), styled(cardTokens(), th)];
    equal(attrOf(attrs, "id"), "x");
    equal(attrOf(attrs, "class"), cls(cardTokens(), th));
    equal(attrOf(attrs, "role"), "");
}

test "the flushed selector matches the class" {
    val th = fullTheme();
...
The rendered round trip — builders and the `html """…"""` DSL producing the same bytes,
`withAttrs` / `attrValue` over a built tree, the flushed selector matching a rendered attribute —
is the `jhonstart-emilia` bridge's test (`modules/jhonstart-emilia/test/bridge_test.bp`, jhonstart
front 30 step 9), the one package that imports both; emilia's tests render no HTML and emilia
carries no dependency on jhonstart (decision 114).

`modules/emilia/test/__snapshots__/class/attributes-the-shared-fixture.snap`
```
e_<hex>
```
This is contract 4's literal. The `jhonstart-emilia` bridge test (jhonstart front 30) and onze 68's
bundle test assert the same string; all three read it from this file, which is why the file is the contract and not the number.
...
```bp
import { Theme, ThemeEntry, Ns, DarkMode, defaultTheme, emptyTheme, extendTheme, clearNamespace, themeValue, themeVar, nsPrefix, darkSelector, darkAtRule, keyframeCss } from "emilia";
...
import { testing: { asserts: { equal, falsy } } } from "std";
...
import { Token, ThemeEntry, emptyTheme, extendTheme, defaultOptions, withTheme, withLayers, withPrefix, withImportant, emilia, flush } from "emilia";
...
```bp
import { Token, ContainerSize, containerKey, containerAt, containerAtMd, containerAt2xl, containerAtSm, containerNamed, ThemeEntry, fullTheme, extendTheme } from "emilia";
...
test "css: container ---- named container shell" {
    val tokens: Token[] = [containerName("page"), .Layout.Block];
    try assertCss(@src(), tokens);
}
...
| 48 | the contract 4 literal, order-is-identity (the rendered round trip is the `jhonstart-emilia` bridge's) | `styled`/`mergeClass`/ASCII equalities, the attribute pair, the flushed selector |
