...
**Replaces:** `1.0.8-beta/01-emilia-layout`
...
The 1.0.8 draft wrote `.Layout.Position.Absolute`, `.Layout.Overflow.Hidden`, `.Layout.Z.10` and
`.Layout.Inset.0` as if they existed. They do not — and that is precisely what this front delivers.
...
## Carried from 1.0.8-beta F01 emilia-layout

The 1.0.8 draft used flat sub-section names and `rem`-resolved offsets; the sections above nest
them and route lengths through front 54's `spacing(n)`. Spelling map (1.0.8 → above):
`Layout.Top.__4` / `Right` / `Bottom` / `Left` → `.Layout.Inset.T.4` / `.R` / `.B` / `.L`;
`Layout.Top.Half` → `.Layout.Inset.T.Frac.Half`; `Layout.Inset.__0` → `.Layout.Inset.All.0`;
`Layout.OverflowX` / `OverflowY` → `.Layout.Overflow.X` / `.Y`;
`Layout.Visibility.Hidden` → `.Layout.Visibility.Invisible` (both emit `visibility:hidden`);
`Layout.ObjectFit` → `.Layout.Object.Fit`; `Layout.ObjectPosition` → `.Layout.Object.Pos`;
`Layout.BreakAfter` / `BreakBefore` / `BreakInside` → `.Layout.Break.After` / `.Before` / `.Inside`;
`Layout.BoxSizing` → `.Layout.Box`; `__N` → `N` in expression position. Everything mapped this way
is covered above. The items below are not.

### 1.0.8 header — the per-utility Tailwind reference list

Complements the **Reference** header line above, which cites `§ 5` and the display page only.
...
### 1.0.8 § Step 1 — `Columns { 4 … 12 }`

Complements § Token surface › *Multi-column and fragmentation* and § Reference gaps, which declare
`Columns` `1`, `2`, `3`, `Auto` plus the named widths and defer `columns-4 … columns-12` until
confirmed upstream. The 1.0.8 draft declared the full integer run:
...
The 1.0.8 example exercises `.Layout.Columns.__4`, `.Layout.Columns.__6`, `.Layout.Columns.__12`.
Upstream v4 `theme.css` does not name `columns-4 … 12` as theme keys — they resolve through the
bare-integer rule — which is why they are a reference gap above rather than a declared leaf.

### 1.0.8 § Step 1 — the offset scale as the draft cut it

Complements § Step 2's `Inset` ladder (`0, …, 96, Px, Auto, Full, Frac { Half, Third, TwoThirds },
Neg { … }`). The 1.0.8 draft used a six-step subset plus three keywords, and no `Neg`:
...
Note the 1.0.8 `Inset` had `Auto` and `Full` but not `Half`, and there were no `X`/`Y`/`S`/`E`
forms; all of those exist above.

### 1.0.8 § Step 1 acceptance — verbatim

Complements § Step 2–5 acceptance above. Two rows differ in emitted text and the form above wins:
1.0.8 wrote `aspect-ratio:16/9` (above: `aspect-ratio:16 / 9`, spaces around the slash as § 5.1
prints them) and `Layout.Visibility.Hidden` (above: `.Layout.Visibility.Invisible`).

- [ ] `botopink test` green with new tests covering every sub-section
- [ ] CSS emitted for `Layout.Position.Absolute` is `position:absolute`
- [ ] CSS emitted for `Layout.Overflow.Hidden` is `overflow:hidden`
- [ ] CSS emitted for `Layout.Z.__50` is `z-index:50`
- [ ] CSS emitted for `Layout.Aspect.Video` is `aspect-ratio:16/9`
- [ ] CSS emitted for `Layout.Inset.__0` is `inset:0`
- [ ] CSS emitted for `Layout.Visibility.Hidden` is `visibility:hidden`
- [ ] CSS emitted for `Layout.ObjectFit.Cover` is `object-fit:cover`
- [ ] CSS emitted for `Layout.BoxSizing.Border` is `box-sizing:border-box`
- [ ] Regression test: existing tokens (Block, Flex, Grid, Hidden) keep working

### 1.0.8 § Step 2 — sub-dispatchers exhaustive without a wildcard arm

Complements § Step 6 (`val out = case …; return out;`, arrow arms, `th: Theme`), which does not
state the no-`_` rule. 1.0.8 named them `layoutPositionToCss`, `layoutOverflowToCss`, etc., each
a `case` over its sub-section.

- [ ] the `tokenToCss` switch has arms for every new sub-section — *above: no new top-level arm;
      `layoutTokenToCss` gains one arm per sub-section instead*
...
- `tokens.bp`: +~120 lines (new sub-sections in Layout)
- `emilia.bp`: +~200 lines (sub-dispatchers)
- `test/layout_test.bp`: new file, ~30 tests
- No other front affected (isolated section)

### 1.0.8 § Notes

All four are covered above (`Inset` calls `spacing(n)` — § Mechanism; `Z` is a bare integer —
§ Step 3; `inset` is the shorthand, `Inset.X`/`.Y` expand — § Step 2; `Float.Start/End` →
`inline-start`/`inline-end` — § Step 4). Carried verbatim:

- `Top`, `Right`, `Bottom`, `Left` use the same scale as spacing (1=0.25rem, 4=1rem, etc.)
- `Z` uses absolute values (10, 20, 30, 40, 50) as in Tailwind
- `Inset` is shorthand for top+right+bottom+left simultaneously
- `Float.Start/End` map to `float:inline-start/inline-end` (Tailwind v4)

### 1.0.8 example tests

The 1.0.8 example ends with 10 inline tests named `"<group> utilities generate correct CSS"`
(`display` → `display:flex`, `position` → `position:absolute`, `overflow` → `overflow:hidden`,
`z-index` → `z-index:50`, `float` → `float:right`, `object-fit` → `object-fit:cover`,
`aspect-ratio` → `aspect-ratio:16/9`, `columns` → `columns:3`, `visibility` → `visibility:hidden`,
`inset` → `inset:0`).

Examples carried:
- [`./examples/layout-example-1.0.8.bp`](./examples/layout-example-1.0.8.bp) — the 1.0.8
  `layout-example.bp`; a same-named 1.0.9 file already exists and differs (`cmp`), so the 1.0.8
  one is kept beside it with the `-1.0.8` suffix. It uses the 1.0.8 spellings in the map at the top
  of this section.
