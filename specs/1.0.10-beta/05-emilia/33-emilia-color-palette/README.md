...
**Replaces:** `1.0.8-beta/14-emilia-color-palette`
...
## Carried from 1.0.8-beta F14 emilia-color-palette

The 1.0.8 draft resolved every shade to a literal hex value; 1.0.9/1.0.10 (§ Mechanism, § Step 3)
resolves it to `var(--color-<family>-<shade>)` and moves the numeric values into
`paletteEntries()` as OKLCH theme entries. The items below are what the 1.0.8 draft carried that
the sections above no longer spell out. They are recorded for completeness, not as a second
conformance target — where the two disagree, the sections above win.

### 1.0.8 § Step 3 — the literal hex ladder

Complements § Step 3 above (`paletteEntries()`), which pins only the two OKLCH values the reference
prints. The 1.0.8 draft pinned the Tailwind hex palette per shade; none of these hex values appear
above.

```bp
// Example mapping (real Tailwind values):
// Red 50: #fef2f2
// Red 100: #fee2e2
// Red 200: #fecaca
// Red 300: #fca5a5
...
1.0.8 acceptance (`__N` is the 1.0.8 expression spelling; the sections above use bare `N` in
expression position and `__N` only in `case` patterns):
...
The 1.0.8 example additionally asserted `Bg.White` → `background-color:#ffffff`,
`Bg.Black` → `background-color:#000000`, `Color.Hex("#abc123")` → `color:#abc123` and
`Color.Purple.__700` → `color:#7e22ce` (the last one in the F15 modifiers draft). Above, `White`
and `Black` emit `var(--color-white)` / `var(--color-black)` and the legacy `Bg.White` path keeps
`background:#ffffff` (§ Step 2).

### 1.0.8 § Blast radius

Absent above — the 1.0.9/1.0.10 template has no blast-radius section.

- `tokens.bp`: +~300 lines (massive Color/Bg expansion)
- `emilia.bp`: +~400 lines (CSS values for every shade)
- `test/colors_test.bp`: new file, ~50 tests
- Every front that uses colours (F05, F06, F07, F12, F13 in the 1.0.8 numbering) depends on this palette

### 1.0.8 § Notes

Two of the three notes are covered above (`Transparent` is a bare variant, § Step 1; opacity is
`Alpha`, § Step 4). Carried for the record, including the design difference on where opacity lives:

- Hex values are from the official Tailwind CSS v4.3 palette
- `Transparent` is added as a simple variant (no payload)
- Opacity modifier (e.g. `Color.Red.__500/50`) is handled in F15 (modifiers) — *1.0.10 instead
  keeps opacity in this front as the top-level `Alpha(percent, inner)` wrapper (§ Step 4); front 34
  owns no opacity form*

### 1.0.8 example tests

The 1.0.8 example ends with 19 inline tests named `"<utility> generates correct CSS"`
(`color-red-50`, `color-red-500`, `color-red-950`, `color-blue-500`, `color-orange-500`,
`color-green-500`, `color-sky-500`, `color-indigo-500`, `color-violet-500`, `color-pink-500`,
`color-slate-500`, `color-zinc-500`, `color-transparent`, `color-hex`, `bg-red-500`, `bg-blue-500`,
`bg-white`, `bg-black`, `bg-transparent`), each asserting `styles.indexOf("<decl>") != -1` on the
hex form. The example also exercises `.Border.Color.Red.__500`, `.Border.Color.Blue.__500`,
`.Border.Color.Gray.__300`, `.Border.Color.White`, `.Border.Color.Black`,
`.Border.Color.Transparent`, which this front does not own (border colour is front 40 territory).

Examples carried:
- [`./examples/color-palette-example.bp`](./examples/color-palette-example.bp) — the 1.0.8
  example, copied unchanged (no same-named file existed). It uses the hex-era spellings `__N`,
  `.Bg.Red.__500` and `.Border.Color.*`; the two 1.0.9 examples above use `.Color.Red.500` and
  `.Bg.Color.Red.500`.
