...
| F3 | **Open** — the transitive dependency closure belongs to `00 · 25-gate-perf` step 4's `loadDependencies` change. A dependency's own `dependencies` are not followed (path or workspace), and the consumer's map is loaded in the order written: `jhonstart-emilia` listed before `emilia` gives `unbound variable 'flush'` inside the bridge | app deps `{ "jhonstart-emilia": {path}, "emilia": {path} }` | each member lists the jhonstart chain first, in dependency order |
