...
pub fn assertStyleSink(loc: SourceLocation, script: string) -> @Result<void, string>
...
```
// GENERATED by onze build — do not edit.
import {readPayload, hydrateIsland, registerFill, checkStyles} from "jhonstart";
import {__jhLinkMount, __jhFormMount} from "jhonstart";
...
== hole h1 with no [data-onze-h]
error: hole h1 is in the payload and not in the document
== order
registerFill < first hydrateIsland < __jhLinkMount < __jhFormMount
```
...
The `script` argument of `assertStyleSink` is a line-oriented program the helper drives against a
real sink: `open <links>`, `register <class>=<rule>` (calls `emilia` under the hood), `head`,
`chunk <holeId>`, `close`.

```bp
import {assertStyleSink} from "onze-test";

test "sink: head ---- the shell registers two classes" {
    try assertStyleSink(@src(),
        \\ open stylesheet=/_onze/static/b7f2a1/app.2b91cc.css
        \\ register e_3f9a1c=background-color:#fff;color:#111827;padding:1rem
        \\ register e_77b0d4=display:flex;gap:1rem;padding-top:.5rem;padding-bottom:.5rem
        \\ head
        \\ close
    );
}
...
`modules/onze-assets/test/__snapshots__/sink/head-the-shell-registers-two-classes.snap`

```
== head
<link rel="stylesheet" href="/_onze/static/b7f2a1/app.2b91cc.css">
<style>.e_3f9a1c{background-color:#fff;color:#111827;padding:1rem}.e_77b0d4{display:flex;gap:1rem;padding-top:.5rem;padding-bottom:.5rem}</style>
== emitted
e_3f9a1c, e_77b0d4
== close
(empty)
...
```bp
test "sink: empty ---- links only, no empty style element" {
    try assertStyleSink(@src(),
        \\ open stylesheet=/_onze/static/b7f2a1/app.2b91cc.css
        \\ head
        \\ close
...
`modules/onze-assets/test/__snapshots__/sink/empty-links-only-no-empty-style-element.snap`
...
```bp
test "sink: chunks ---- a late boundary carries its own block before its markup" {
    try assertStyleSink(@src(),
        \\ open
        \\ register e_3f9a1c=background-color:#fff
        \\ head
        \\ register e_19ee42=opacity:.5
        \\ chunk h0
        \\ chunk h1
        \\ register e_c5e2a0=font-size:.875rem
        \\ chunk h2
        \\ close
    );
}
...
`modules/onze-assets/test/__snapshots__/sink/chunks-a-late-boundary-carries-its-own-block-before-its-markup.snap`

```
== head
<style>.e_3f9a1c{background-color:#fff}</style>
== chunk h0
<style data-onze-s="h0">.e_19ee42{opacity:.5}</style>
== chunk h1
(empty)
== chunk h2
<style data-onze-s="h2">.e_c5e2a0{font-size:.875rem}</style>
== emitted
e_3f9a1c, e_19ee42, e_c5e2a0
== close
(empty)
```

```bp
test "sink: errors ---- a second head, a non-empty close, a </style in a body" {
    try assertStyleSink(@src(),
        \\ open
        \\ head
        \\ head
        \\ register e_00bad0=content:"</style><script>"
        \\ close
    );
}
```

`modules/onze-assets/test/__snapshots__/sink/errors-a-second-head-a-non-empty-close-a-style-in-a-body.snap`

```
== head
(empty)
== head
error: collectHead called twice for this render (second call refused)
== close
error: closeSink: 1 class registered after its chunk was collected (e_00bad0)
error: style body contains "</style" (refused, source: register e_00bad0)
```
...
| 69 | 31 | head, empty, chunks, double head, close, `</style`, scoped names + set equality + undefined class, cascade + fingerprint + `Y` round trip, resolution table + two prefixes + 304 + adversarial config + content types | `renderDocument` byte-identity with the sink (E2E), preprocessor hook (`process.run` doubles, `assert`) |
