...
| A matched route becomes a rendered page | rakun → jhonstart, or jhonstart → rakun | jhonstart's `#[page]` / `#[layout]` / `#[template]` decorators (front 30), each on a `#[@use] … -> @Component<Element>` (decision 117), fill jhonstart's UI registry; `Onze.run` copies the records into rakun's table and registers one opaque `PageRenderer` per page (rakun 23, decision 114); rakun matches (22, through the bundled `routing`) and calls the renderer with its `Request` and `ChunkWriter`; the renderer calls `site.renderStream(input, requestData(req), res)`, so jhonstart receives the segment chain, the payload's rakun-side strings (`t`, `a`, `b`), the `RequestData` its `headers()` / `cookies()` read, and a `Response` over rakun's `ChunkWriter`; jhonstart's router imports the same matcher from `routing` (decision 115); jhonstart's `notFound` / `redirect` signals are handled by jhonstart's render alone — 404 / 307 through the `Response` before the first chunk, markup after it (status 200) — and onze has no `case` on a signal (decision 117) |
