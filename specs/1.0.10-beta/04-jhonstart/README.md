...
| **26** | [`26-jhonstart-router`](./26-jhonstart-router/README.md) | critical | 1 | `jhonstart` | 94 (examples) | 01 std (`encoding`) · std `04-routing-lib` (`matchPath` / `parseTable`, `navigation.signalFromWire`, imported from the bundled `routing`) · std `05-actions-lib` (`refreshValue`) · 22 rakun file-routing (the format) *(ro)* | erlang |
...
| **30** | [`30-jhonstart-streaming`](./30-jhonstart-streaming/README.md) — render and streaming | high | 4 | `jhonstart` · `jhonstart-emilia` | 28 · 29 · 31 · 32 · 94 | 01 std (`escape`) · 07 std json-writers (`json.*`, `escape.scriptJson`) · 02 std async (spawn/gather over thunks) · std `04-routing-lib` (`navigation`, `matchPath` for late signals) · emilia `flush()` (bridge only) | both |
