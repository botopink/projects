...
**jhonstart writes the HTML** (decision 113): the render, its escaping, the document, the payload, islands, streaming, links, hydration, the client router (with `clientApp`, the client-only entry), the navigation signals end to end and the UI file conventions (`#[page]`, `#[layout]`, `#[template]` — each a `#[@use] fn … -> @Component<Element>` — `PageContext`, `LayoutProps`; decisions 114 and 117) are this track's, and emilia's CSS enters through the render-plugin point and the `jhonstart-emilia` bridge member. jhonstart and rakun never import each other; onze is the only package that names both.
