...
├── modules/
│   ├── jhonstart/                     CORE — the app model front 23 renders           target: both
...
One criterion decides core versus submodule, and it is checkable by grep: **core is what the rakun SSR pipeline imports.** Fronts 22 (`Loading`, `ErrorPage`, `NotFound` discovery), 23 (`renderNode` over `Element`, `isVoidTag`/`isRawTextTag`, `Boundary`/`Suspense`/`resolve`/`fillHtml`, `renderBoundaryChecked`, `Island`/`islandEntry`, `renderHead`/`mergeMetadata`, `RouterState`), 62 (`request()` binds its module), 63 (`isSignal`), 66 (`openGraph.images`) reach into jhonstart by name. Everything they reach is core. `Link`, `reconcile`, `formAttrs`, `actionState` are reached only by application components and by front 68's generated entry — they are submodules.
...
| `jhonstart` | `suspense.bp`, `streaming.bp` | 30 | — |
...
| `Boundary`, `Suspense`, `holeId`, `Chunk`, `resolve`, `fillHtml`, `shellHtml` (30) | `from "jhonstart"` |
| `ErrorInfo`, `ErrorBoundary`, `renderBoundary`, `renderBoundaryChecked`, `catchError`, `infoFor`, `serverInfoFor`, `digestOf`, `isSignal`, `wrap` (31) | `from "jhonstart"` |
...
| `Link`, `LinkProps`, `linkProps`, `withPrefetch`/`withReplace`/`withScroll`/`withTarget`/`withClass`, `prefetchMode`, `layoutKey`, `layoutKeys`, `sharedDepth`, `reconcile`, `LinkStatus`, `linkStatus`, `__onzeLinkMount`, `__onzeLinkPrefetch` (27) | `from "jhonstart-link"` |
| `ActionState`, `newActionState`, `parseActionState`, `FormBinding`, `formAction`, `formAttrs`, `hiddenActionField`, `actionState`, `FormStatus`, `formStatus`, `optimistic`, `applyOptimistic`, `SearchFormProps`, `searchFormProps`, `searchFormAttrs`, `__jhFormMount` (67) | `from "jhonstart-forms"` |
...
|---|---|---|
| `jhonstart` (core) | **keep** | Holds every module front 23 renders through. Eight fronts deliver into it (26 · 28 · 29 · 30 · 31 · 32 · 94 + frozen v0). Target both: every module renders on erlang (server pass) and re-renders on js. |
...
| `jhonstart-link` | **keep** | Front 27 alone: `link.bp` + `reconcile.bp`. js target — its four cells are `#[@External.Node]` with no erlang twin; nothing in core or in the rakun pipeline imports it; a statically generated site (front 60) or a no-bundle server render ships without it. Depends on core (26's `RouterState.segments()`, 94's `a`). The pure `Link` render still runs on erlang, which is why the submodule's manifest declares both targets and its gate is commonJS. |
...
| `jhonstart-client` (29) | **merge into core** | `Island`, `clientMount`, `islandEntry`, `serverSlot` are called by front 23 during the BEAM pass to build the payload's `i` rows; `#[client]`/`#[clientProps]` are applied in component files that are server-rendered. The one browser cell, `hydrate()`, is the per-island entry point front 68's generated module imports — one import from core. Splitting 29 would make the server pipeline depend on a submodule named "client". |
| `jhonstart-streaming` | **merge into core** | `Suspense`/`resolve`/`fillHtml`/`shellHtml` are front 23's flush contract; erlang; imports `Element` only. Two files. |
| `jhonstart-errors` | **merge into core** | `renderBoundaryChecked` is what front 23 calls around every segment; `isSignal` is front 63's discriminator; `global-error.bp` needs 94's `htmlTag`/`body`, both core. |
| `jhonstart-metadata` | **merge into core** | `renderHead`/`mergeMetadata` run inside front 23's document build; imports `pairValue` (core) and std `escape`. |
| `jhonstart-forms` | **keep** | Front 67 alone: `form.bp` + `form_state.bp`. js target; the deepest front in the track; the only jhonstart code pinned to a **rakun** contract literal (front 24's envelope and golden `state` fixture). A read-only site ships without it. Depends on core (`push`, `form`/`input`/`button`/`label`) and on `jhonstart-link` (`__onzeLinkPrefetch` for the GET `<Form>`). Nothing depends on it. |
| `jhonstart-elements` (94) | **merge into core** | Front 94's own acceptance: "a reader cannot tell from a call site whether a tag came from `element.bp` or from `elements.bp`", and `isVoidTag`/`isRawTextTag` are consumed by front 23's `renderNode`. |
...
Net: **five directories** — core, `-html`, `-link`, `-forms`, `-test`.
...
```
              ...
              │   client(29) · suspense/streaming(30) · error_boundary(31) · metadata(32)│
              └───────┬──────────────────┬───────────────────────┬──────────────────────┘
                      │                  │                       │
                      ▼                  ▼                       ▼
             jhonstart-html      jhonstart-link (27)     rakun 22 · 23 · 24 · 62 · 63 · 66
             html · html_attrs(48)        │                       │
                      ▲                   ▼                       │
...
- **rakun → jhonstart, never the reverse.** `server.bp` binds `rakun_request_context` by `#[@External.Erlang]` module name — a host-module string, not a package import — so jhonstart's manifest lists no rakun dependency. Front 31's `not-found-example.bp` imports `notFound` from `"rakun"`; that is an example project's dependency (§ 7), not the library's.
- **emilia → jhonstart-html, one way.** Front 48 owns `html_attrs.bp` inside this repo and asserts by test that the string `emilia` appears nowhere under `repository/jhonstart/`. The attribute slot is emilia-unaware plumbing; emilia's `html_hook.bp` is the side that knows both.
- **Front 68's generated entry** imports `hydrate` from core, `__onzeLinkMount` from `jhonstart-link`, `__jhFormMount` from `jhonstart-forms` — three imports, one call each, in that order.
...
Host cells by submodule — the rule the manifests encode: core carries `#[@External.Erlang]` cells (26's five, 28's six) plus exactly two `#[@External.Node]` cells (26's dual-target `__jhNavigate`, 29's `hydrate`/`__onzeClientPropsRaw`); `-link` and `-forms` carry `#[@External.Node]` only and **no** `#[@External.Erlang]` cell, asserted by grep in each submodule's gate.
...
| **30** streaming | `modules/jhonstart/src/` | `suspense.bp`, `streaming.bp` | `assert_stream.bp`, `renderToStream` | erlang |
...
|---|---|---|---|
| emilia (D) | **48** attributes | 48 owns `modules/jhonstart-html/src/html_attrs.bp` — the only cross-repo file. jhonstart stays emilia-unaware; `[class]={expr}` reaches `attrs` (`html.bp:234-241`) and emilia's `html_hook.bp` produces the class string. | a `#("class", "e_<hex>")` pair, nothing else |
| rakun (B) | **23** ssr-pipeline | Imports core: `Element`, `isVoidTag`, `isRawTextTag`, `Boundary`, `resolve`, `fillHtml`, `shellHtml`, `renderBoundaryChecked`, `Island`, `islandEntry`, `renderHead`, `renderViewport`, `mergeMetadata`, `RouterState`. Renders with its own `renderNode` — never `renderToString`. | the `__onze` payload (`contracts.md § 2`) it builds from what core hands it |
| rakun (B) | **24** server-actions | Builds its `actionForm` from 94's `form`/`input`/`button`; owns the action id and envelope that `jhonstart-forms` decodes. | the golden `state` fixture, asserted verbatim on both sides |
