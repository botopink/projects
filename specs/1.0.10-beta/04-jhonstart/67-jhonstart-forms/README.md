...
| Tuple destructuring from a `use` binds fresh type vars — `val #(shown, push) = use optimistic(…)` parses and lowers, but `shown`/`push` take no type from `R`; front 01 records the labeled half | `optimistic-like-example.bp` — `shown` is typed only by flowing into `likeWidget`'s `i32` | the destructure, with each element used where its type is fixed; or `o._0` | bind each element to `R`'s tuple element — [`19-use-activation`](../../00-compiler-carry-over/19-use-activation/README.md) step 3 |
