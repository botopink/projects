...
cite). Wave B — [`06-comptime-dedup`](../06-comptime-dedup/README.md) step 2 landed, and step 1
...
([`06-comptime-dedup`](../06-comptime-dedup/README.md)) · `src/parser/tests/**` ·
...
[`06-comptime-dedup`](../06-comptime-dedup/README.md) step 1: a `duplicate` or `wrong-test` fix in
...
| wave B after **07 step 2** (the renderer) | unchanged in substance: it is now [`06-comptime-dedup`](../06-comptime-dedup/README.md) step 2. Step 1 is a *new* additional precondition for any **rename**, which 1.0.4-beta only noted in its blast radius |
...
[`06-comptime-dedup`](../06-comptime-dedup/README.md) step 2, which turns many `weak` rows into `ok`
...
[`06-comptime-dedup`](../06-comptime-dedup/README.md) step 1** — a rename moves four files until then.
...
[`06-comptime-dedup`](../06-comptime-dedup/README.md) step 1 deletes `comptime/{node,erlang,beam,wasm}/`.
...
- **Test renames move snapshot files.** In `src/codegen/tests/**` a rename moves four files (one per
  ...
  [`06-comptime-dedup`](../06-comptime-dedup/README.md) step 1 and one after**; in
...
- The three copies of `slugify`/`slugFromSrc` (`codegen/tests/helpers.zig`,
  ...
  `comptime/tests/helpers.zig` is [`06-comptime-dedup`](../06-comptime-dedup/README.md)'s, so the merge
...
- The six content-identical comptime error pairs listed in
  [`06-comptime-dedup`](../06-comptime-dedup/README.md#current-state) are `duplicate` rows of this
