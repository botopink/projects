...
| `snapshots/comptime/**` — 20 `COMPTIME ERLANG` cells | [`../06-comptime-dedup/`](../06-comptime-dedup/README.md) restructures the directory | **sequencing choice, measured below** |
...
1.0.4-beta — [`../fronts.md` § Unowned items](../fronts.md#unowned-items)) · the eval-protocol half of
...
`src/comptime/snapshot.zig` ([`../06-comptime-dedup/`](../06-comptime-dedup/README.md)) · the module
...
```markdown
| [`14-comptime-on-beam`](./14-comptime-on-beam/README.md) | **first** | steps 0–2 start now (two named carve-outs); step 3 deferred until 03 closes | Comptime evaluation renders an Erlang **source** module per call site and compiles it in the resident node: 8–52 ms of `compile:file` to run a body that takes 0.05 ms, and 200 call sites of one template produce 200 copies of one program. Costs the options — `.S` + `erlc +from_asm`, `.beam` from Zig, one module per declaration — and recommends compiling once per declaration first |
```

### Unowned items this front takes over

```markdown
| ~~`src/comptime/template_eval.zig`, `src/comptime/decorator_eval.zig` (owned by 1.0.2-beta comptime-dispatch, landed)~~ — **claimed by [`14-comptime-on-beam`](./14-comptime-on-beam/README.md)** | — | — | 14 |
