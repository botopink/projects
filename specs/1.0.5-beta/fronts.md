...
| **09** [`ecosystem-residuals`](./09-ecosystem-residuals/README.md) | `repository/{emilia,erika,jhonstart,onze,rakun}/**`; their meta submodule pointers | the libraries' own test outputs and examples | **step 1 landed 2026-09-18** — the five libraries are formatted and committed (emilia `3e7ab05`, erika `02f4344`, jhonstart `78e01ca`, rakun `6567b14`, onze already clean): 874 changed lines, token-stream equality per file, byte-identical emitted output per example, `test-libs` 11 passed / 0 failed. **Step 2 is superseded by decision 17**; step 3 after 01, step 4 after 13. Also landed: **`libs/std/src/beam.bp`**, decision 43's layer 1 — ten `#[@External.Erlang]` primitives and `pub mod beam;` in `root.bp` — committed and gate-green **before front 17 opens**, as [decision 62](./decisions-taken.md#62-the-order-of-what-is-left-in-the-milestone) ordered. It is a file this row's *owned* column does not list, so the carve-out wants writing down. What it measured is [question 64](./decisions-pending.md#64-how-does-beammemorys-layer-2-reach-layer-1-when-the-erlang-backend-emits-no-wrapper): the erlang backend emits no wrapper for a host-bound std `declare fn`, so nothing on the BEAM can call layer 1 yet |
...
| **12** [`language-tests`](./12-language-tests/README.md) | `repository/botopink-lang/tests/language/**` — the cells, `expected-failures.txt`, `run.sh`, `AGENTS.md` | — | **landed twice, 2026-09-18** — first pass: step 1 had already landed at `7ab6a55`, so nothing was blocked; the header became a run (250 passed / 61 expected / 0 failed, beam 14/20/0), `AGENTS.md`'s not-parsing table lost 5 of its 7 rows, and 7 cells cover decisions 28/30/33. Four defects found, one of them beam **dropping an index silently with exit 0**. Second pass: eight cells for decisions 52–55, three false wasm reason texts corrected, and `test/loop_collection.bp` — a cell that **passed on all four backends** — rewritten, because decision 55 ends the loop at a value `break` and all four shared one accumulator that never stopped. It also re-measured six statements in decisions 52–55 and 59 that are now corrected in [`decisions-taken.md`](./decisions-taken.md). After the 12 × 13 merge the file reads **77 lines** (66 `--target all`, 11 beam-only, 23 naming a second row) and the suite **268 passed / 66 expected / 0 failed**, `--target beam` 19 / 23 / 0 |
