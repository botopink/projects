...
of [`10-cli-residuals`](../10-cli-residuals/README.md) step 1**, which is implemented and parked. Step 5
...
comptime layout — and [`06-comptime-dedup`](../06-comptime-dedup/README.md) step 1 deletes that block
...
[`06-comptime-dedup`](../06-comptime-dedup/README.md) step 1 deletes. Do not edit the file; verify
after that front lands, and record the two conditions that are already met.

**Acceptance:**
- [ ] After [`06-comptime-dedup`](../06-comptime-dedup/README.md) step 1:
...
2. Decide and act on the `libs/std` warning. Options, in increasing cost:
   ...
   | B | Exempt a module listed in `botopink.json`'s `files` from the "not reached by any `mod` path" check | Correct by the manifest's own meaning — `files` is the declared surface — but it is a CLI change, [`10-cli-residuals`](../10-cli-residuals/README.md)'s file |
   ...
   **Recommended: B**, handed to [`10-cli-residuals`](../10-cli-residuals/README.md) as a one-row
...
3. **The `docs.md:78` import fence.** [`10-cli-residuals`](../10-cli-residuals/README.md) step 1 is
...
- [ ] `zig build test-docs` green **with** [`10-cli-residuals`](../10-cli-residuals/README.md) step 1
...
- **Step 3's `docs.md` edit unblocks a front that reds real code.**
  [`10-cli-residuals`](../10-cli-residuals/README.md) step 1 makes an unresolved `import` a located
...
- **5.13's fixture rewrite** (`comptime/tests/infer_decls.zig:540`) re-records one comptime snapshot:
  **four files before [`06-comptime-dedup`](../06-comptime-dedup/README.md) step 1, one after.** Land
  it after that step.
...
| Step 2.2 option B | the "not reached by any `mod` path" check | [`10-cli-residuals`](../10-cli-residuals/README.md) — handed over, not taken |
| Step 3.3 | `docs.md:78` | this front; the edit is specified by [`10-cli-residuals`](../10-cli-residuals/README.md) |
...
1. **`docs.md:78`** — the skip directive or the richer `project modules` rewrite (step 3.3). Until one
   lands, [`10-cli-residuals`](../10-cli-residuals/README.md) step 1 cannot be committed.
...
- `modules/compiler-core/AGENTS.md:19-20` describes the snapshot tree as
  ...
  [`06-comptime-dedup`](../06-comptime-dedup/README.md)'s to rewrite, not this front's — it is listed
