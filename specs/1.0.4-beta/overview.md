...
| [`16-module-naming`](./16-module-naming/README.md) | high | not started — after 01 closes | The erlang/BEAM module atom is the source path's basename, so two modules with the same file name collide silently and eleven `libs/std` modules shadow an OTP module. An unquoted `@`-joined path atom (`std@math`, `web@api@http`), a `__t__`/`__b__` qualifier for the modules a file emits beyond its own, and — decided 2026-09-17, in the same front — [policy 3](./16-module-naming/policy-3-module-per-type.md): every `type` and `behavior` gets its own BEAM module, which ends the name mangling and makes a stack trace name the owner |
