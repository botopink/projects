...
| **The libraries' example gate aborts silently on a `known-broken-examples.txt` holding only comments** (`grep` exits 1 under `set -euo pipefail`) — the lists were deleted in emilia and jhonstart instead | `scripts/git-hooks/lib/runner-standalone.sh` (byte-identical in the five libraries) | 13 examples fix | a one-line `\|\| true`, in the five libraries (13, or now as a follow-up) |
