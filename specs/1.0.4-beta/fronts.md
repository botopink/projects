...
|---|---|---|---|
| `Array.chunked` / `Array.sliding` are written with `while`, which checked code rejects as not in scope — **decided 2026-09-17: rewrite them with `loop`; `while` is not part of the language** — a `while (…) { … }` call is a check error and commonJS's `while` special case is deleted (06 N-row) | `libs/std/src/primitives.bp` | 1.0.4-beta erlang | assigned by the maintainer as a one-off `libs/std` edit (like std-split) |
...
| **`hover_interface_method`** — **decided 2026-09-17:** the footer names the declaring interface and the receiver's, `*from interface Signed (via I32)*` | `modules/language-server/src/engine.zig` | 1.0.2-beta review-tooling (report 3.12) | 14 (it owns user-facing `engine.zig` texts), or earlier as a one-line follow-up |
