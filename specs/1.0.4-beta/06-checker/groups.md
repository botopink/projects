...
> Carried from `1.0.2-beta/07-checker/` into 1.0.4-beta. `file:line` and counts were measured at that milestone's
> commit — re-locate by symbol. `F1…F12` and bare front numbers are the old numbering: see
> [`../fronts.md`](../fronts.md#old-front-numbers).
>
> The library measurements below predate the comptime-dispatch and std-surface landings — see
> [`README.md`](./README.md) § Current state before quoting them.
...
| **G3 — strictness** | C9 | **79 method bodies** — erika 39, std 31, rakun 7, onze 2 | Needs the measurement in its row, which needs std and erika compiling (G0's C6, and `comptime-dispatch` (1.0.2-beta, landed) for erika) and needs G1 landed — many swallowed errors are `return`- and `case`-shaped |
...
|---|---|---|---|
| `libs/std` | 30 files / 4823 lines | **red** — `error: pick expects a type and field names at random:141:13` | **C6** (and `std-surface` (1.0.2-beta, landed)) |
| erika | 3 / 1031 | **red** — `the template module did not compile` | `comptime-dispatch` (1.0.2-beta, landed) |
| rakun | 15 / 1064 | **red** — `a declared dependency was not found under the libs root` | [`10-library-repos`](../10-library-repos/README.md) |
...
**Cross-front decision, due before `std-surface` (1.0.2-beta, landed) lands.** That
...
| erika | C1 1 | **not until `comptime-dispatch` (1.0.2-beta, landed) lands** |
| onze, rakun | 0 | onze today; rakun after [`10-library-repos`](../10-library-repos/README.md) |
...
| erika compiles | 39 of the 79 bodies — the largest single exposure in the front, never type-checked (`erika/src/erika.bp:24` `pub fn toArray(self: Self) -> Array<T>`, `:105` `return Query(items: sorted);` inside `orderBy<K>`) | **`comptime-dispatch` (1.0.2-beta, landed) — erika's 39 cannot be measured at all before it lands** |
| rakun compiles | 7 of the 79 | [`10-library-repos`](../10-library-repos/README.md) |
...
`comptime-dispatch` (1.0.2-beta, landed) lands.
...
| Types as values, A0…A5 | [`types-as-values.md`](./types-as-values.md) | A0 is `std-surface` (1.0.2-beta, landed)'s; A1 lands after C10's two-pass registration (G1) and deletes C10's bindings arm; A5 lands after C6's shadowing fix (G0) |
...
**Interaction with [`07-comptime-dedup`](../07-comptime-dedup/README.md).** That front deletes 3
