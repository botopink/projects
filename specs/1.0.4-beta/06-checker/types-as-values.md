...
> **Changed since this was written.** The 1.0.2-beta std-surface front **deleted** `types.bp` and
> `reflect.bp` (its step 6) and landed C6's shadowing guard (its step 1), so A0 as written no longer
> exists: the Zig intercept is the sole definition of `partial`/`omit`/`pick`/`mergeRecords`, and
> A4/A5's `.bp` half means writing those functions afresh. `libs/std/**` has no owner this
> milestone — agree who writes that half before starting A4. The comptime evaluator files
> (`comptime/template_eval.zig`, `decorator_eval.zig`) likewise have no owner now that
> comptime-dispatch has landed; A4's `erl` path may claim them.

A0 edits only `libs/std/**`, which `std-surface` (1.0.2-beta, landed) owns; it is that
...
`comptime/decorator_eval.zig`, which `comptime-dispatch` (1.0.2-beta, landed)
...
`std-surface` (1.0.2-beta, landed); the compiler half (the deletions) is here. They
