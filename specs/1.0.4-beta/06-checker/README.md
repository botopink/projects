...
| N28 | **A section of an enum-shaped `type` is a type named by its path** (decision 8 §5.3b, decided 2026-09-17): `Token.Text`, `Token.Text.Size` resolve in type position; a pattern binding a section gives that type (C8), and the 27 undeclared section annotations of emilia (C10) become path names — C8 and C10 land with emilia's migration ([`../13-ecosystem-migration/`](../13-ecosystem-migration/README.md)) | G1 | decision 8 §5.3b |
| N29 | **A section value only writes as the leading-dot form with context** (`.Text.Bold`); the qualified `Token.Text.Bold` is refused — the value side does not mirror the type path that N28 gave the language (decision 8 §5.3b) | G1 | 06 N28 implementation, 2026-09-17 |
| N30 | **`TypeRef` carries no `Loc`**, so an unknown type name reds at the file, not at the annotation — C10's core (two-pass registration) needs it; keep forward references, imported typedefs and the compiler-known `Children` working | G1 | 06 C10, 2026-09-17 |
