...
`comptime/snapshot.zig` itself is owned by [`07-comptime-dedup`](../07-comptime-dedup/README.md),
...
| **Cross-front** | `std-surface` (1.0.2-beta, landed) also names this collision and cannot fix it without renaming a public std function. Decide before that front lands, or it renames `random.pick` and this front renames it back |
