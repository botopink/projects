...
after `comptime-dispatch` (1.0.2-beta, landed) and before quoting any of them.
...
`comptime-dispatch` (1.0.2-beta, landed) is in.
...
| …of which carry a `typeNameOf` `.typeVar` `?` | **17 slugs / 68 files** | the tripwire set — 10× `case_*`, 5× extension/method dispatch, 2× `@makeRecord` (measured by `07-comptime-dedup/renderer.md`) |
...
[`07-comptime-dedup`](../07-comptime-dedup/README.md) removes. If that front lands first, every
...
until `comptime-dispatch` (1.0.2-beta, landed) lands**. In the *suite* the
