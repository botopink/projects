...
# commonJS — a unit variant emitted as a bare string (only if step 6 is taken)
...
| **erlang + beam, steps 2–3** | 627 | **130** | 21 % |
| `snapshots/codegen/commonJS/` | 314 | **13** (step 6 only, and only if the maintainer assigns it here) | 4 % |
| `snapshots/codegen/wasm/` | 313 | **0** — `wat.zig` is not this front's ([`options.md` § 7](./options.md#7-the-other-three-backends)) | — |
...
Contrast [policy 3](../16-module-naming/policy-3-module-per-type.md), whose 188 diffs each gain whole
`----- ERLANG -- <file>.erl` sections. That is why the two fronts sequence
([`front-16.md`](./front-16.md) § 4) rather than merge.

## 2. Overlap with the other fronts' snapshot sets

```console
$ grep -rl '^----- COMPTIME ERLANG' . | wc -l          # front 18's set
48
```
...
| **16** policy 3 (188 cells carrying a `%% type`/`%% behavior`/`%% implement` marker) | **large** — 81 of the 314 erlang cells carry the marker and 62 of them build a value | must sequence; 16 first |
| **01** step 6 (owns all four codegen directories) | all of them | 19 runs after 01 closes |
| **06** (may move all four codegen directories) | all of them | 19 runs after 06 |
...
- **Step 5 (§7's formatter) also changes no existing `RUN LOG`.** Decision 8 §7's new text
  ...
  `tests/language/` ([`../15-language-tests/`](../15-language-tests/README.md)'s directory,
  [`../17-language-test-expansion/`](../17-language-test-expansion/README.md)'s to extend).

One `RUN LOG` class *does* move and is not in the snapshots: the equality of two record values.
Measured on the compiler's own output ([E13](./evidence.md#e13--both-spellings-applied-to-the-compilers-own-output)),
...
| Libraries that start compiling | **0** — this front fixes latent failures, exactly as [16 does](../16-module-naming/README.md#blast-radius); its new cells are the only evidence the work did anything |
...
|---|---|---|
| `src/codegen/crossModule.zig` | `typeAtom` / `variantAtom` beside 16's `erlDeclAtom`; the collision check extended to type and variant atoms | ≈ 40 |
...
| `src/codegen/commonJS.zig`, `typescript.zig` | step 6 only: `commonJS.zig:1563-1566`, `:3874`; `typescript.zig:115-127` | ≈ 30 |
| the `is` / union-`case` / formatter lowerings (steps 4–5) | new code in both backends, after 06 | ≈ 250 |
...
Estimated **≈ 3–4 days** for steps 1–3 and 6, **+3–4 days** for steps 4–5 (which cannot start before
06 lands N20/N21/N22), so **≈ 7 days** total. That is on top of 16's ≈ 9.

## 6. What a user sees change

| Before | After | Where it is visible |
|---|---|---|
| `@print(p)` → `#{name => <<"Ana">>,age => 30}` | step 3: `#{name => …,'__bp_type' => main__t__person,age => 30}`; step 5: `Person(name: "Ana", age: 30)` | any program that prints a record on erlang or beam |
| `@print(Shape.Dot)` → `'Dot'` | step 3: `main__t__shape__v__dot`; step 5: `Shape.Dot` | idem |
...
| `out/` layout, module names, `.js` / `.wat` output | unchanged by this front | 16 owns the layout |

**The one user-visible regression risk**: between step 3 and step 5 the printed form of a record is
*worse* than today (it shows the internal key). The two steps should land in the same release even
if they land in different commits, and step 5's acceptance is what closes it.
