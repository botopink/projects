...
| Size, `Person(name, age)` | **13 words** (11 today) | **13 words** — identical: an atom and a small integer are both immediates ([E16](./evidence.md#e16--the-size-of-every-candidate)) | **15 words** (+36 %) | 11 words |
| Size, an enum variant | **free** — an atom is 0 words, the tuple keeps its arity (§ 4) | free | not applicable | free |
| Readable in a **guard / pattern** — decision 8 §3.3, §5.4 | **yes** (E13, E15) | yes | **no — `illegal guard expression`** ([E17](./evidence.md#e17--a-carried-function-cannot-be-read-in-a-guard)) | partly: only shapes, never names |
| Answers `is Person` vs `is Vec` | yes | yes | yes, after a call | **no** — 6 field-sets are already shared by 18 differently-named types ([E14](./evidence.md#e14--how-often-the-names-already-collide)) |
...
| Names the §7 formatter | **yes, for free under [policy 3](../16-module-naming/policy-3-module-per-type.md)** — the atom *is* the module holding `format/1` | no — needs a lookup table the backend must emit and keep in sync | yes — but the call is the dispatch, so the tag and the formatter are welded together and `is` cannot reuse it in a guard | no |
...
| Snapshots it moves | 130 (§ [`blast-radius.md`](./blast-radius.md)) | 130, same lines | 130, and every `~p` of a record prints a fun | 0 |
...
[front 16](../16-module-naming/README.md) is already doing.
...
The first line is [A2](../16-module-naming/declaration-qualifier.md)'s string, reused verbatim —
`erlDeclAtom(alloc, id, .t, decl, null)` from 16 step 1. The second is **new** and is what this
...
([E15](./evidence.md#e15--the-qualified-variant-tag)):
...
([16 E7](../16-module-naming/evidence.md#e7--the-real-length-cap-is-the-filename-not-the-atom)).
...
| an existing `#{x := X}` pattern | **still matches** ([E1](./evidence.md#e1)) | breaks | breaks |
| `is` test | `maps:get('__bp_type', M, undefined) =:= T` ([E3](./evidence.md#e3)) | `element(1, V) =:= T` | `is_record/2` |
| cross-module | works | works | **needs an `.hrl` the compiler emits nowhere** ([16 erlang-atoms § 2.3](../16-module-naming/erlang-atoms.md)) |
...
**T1 for step 2, T2 kept open.** E1 is still the decisive measurement: a map pattern in erlang is a
...
> **Correction to [`evidence.md` E10](./evidence.md#e10--build-and-match-2-000-000-iterations).** E10
...
> ([E18](./evidence.md#e18--the-honest-timing)). Through an opaque call the three `is` lowerings are
...
> either. The 2.5× figure must not be used to justify step 5.
...
([E13](./evidence.md#e13--both-spellings-applied-to-the-compilers-own-output)), that works — and it
...
This also **corrects [`README.md`](./README.md) step 3**, which says "`is_tagged_tuple`'s arity
...
[policy 3 § 3](../16-module-naming/policy-3-module-per-type.md#3-the-cross-module-index) reached for
...
recorded by [16 step 6](../16-module-naming/README.md#step-6--the-residuals-this-front-will-not-take)
