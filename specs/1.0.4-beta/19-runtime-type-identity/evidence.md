...
The tag used throughout is `models@user__t__pessoa` — [option A](../16-module-naming/options.md)'s
`erlAtom("models/user")` joined with [A2](../16-module-naming/declaration-qualifier.md)'s
`__t__<decl>`.
...
[decision 8](../08-review-backlog/decision-8-language.md) §4.2's `x is Point` has a lowering on both
...
after a lowercase start, which is what [option A](../16-module-naming/options.md)'s steps 3–5
guarantee. Nothing in the emitters, the `.S` writer or `beam_export_audit.sh` has to learn quoting
(contrast the `#Pessoa` proposal, [erlang-atoms.md § 2.2 B2](../16-module-naming/erlang-atoms.md)).
...
[`../01-backend-residuals/`](../01-backend-residuals/README.md) currently names.
...
[erlang-atoms.md § 2.1](../16-module-naming/erlang-atoms.md) and
[`../18-comptime-on-beam/`](../18-comptime-on-beam/README.md).)
...
## What was *not* measured, and should be before step 5
...
- `-record` / `is_record/2` (T3) was not benchmarked. It is ruled out on distribution grounds — the
  ...
  ([erlang-atoms.md § 2.3](../16-module-naming/erlang-atoms.md)) — not on speed.
- The beam `.S` side was reasoned from `beam_asm.zig`, not run. `is_tagged_tuple`'s arity argument
  grows by one under T2; step 3's acceptance is where that gets executed.
...
Recorded, not this front's ([policy 3](../16-module-naming/policy-3-module-per-type.md) deletes both).
...
Commands and counts in [`blast-radius.md`](./blast-radius.md). The two results worth repeating here:
...
overlap with front 18's 48 COMPTIME ERLANG cells : 0
```

**Correction to [`README.md`](./README.md) § 2.4.** "Every record `RUN LOG` … the largest item" is
wrong: no snapshot in any backend prints a record or a variant, so steps 2–3 change **no `RUN LOG`
