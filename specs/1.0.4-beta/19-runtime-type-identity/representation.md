...
[`evidence.md` § E11](./evidence.md#e11--one-program-five-backends). Line numbers are at `26d4fdc`
...
| Consequence | Where |
|---|---|
| `Person(name:"Ana", age:30) =:= Vec(name:"Ana", age:30)` is **`true`** — executed, not reasoned ([E11](./evidence.md#e11--one-program-five-backends)) | — |
...
across the seven repositories (115 `.bp` files, [E14](./evidence.md#e14--how-often-the-names-already-collide)):
...
(`isRecordMethodCollision`, `erlang.zig:1868`). [Policy 3](../16-module-naming/policy-3-module-per-type.md)
...
3. The `.d.ts` **disagrees with the `.js` it describes**. `typescript.zig:123` writes
   ...
   ones. Reproduced live ([E12](./evidence.md#e12--the-dts-contradicts-the-js-for-a-unit-variant));
...
4. `p === v` is `false` — but so is `Person("Ana",30) === Person("Ana",30)`. commonJS compares
   ...
   values. Front 15 already pins the same mechanism for tuples
   (`tests/language/expected-failures.txt`, two `tuple_equality.bp` rows, owner 01 step 6).
...
This front does **not** own `wat.zig` ([`README.md`](./README.md) § Owns; decision 3's box is
[`../01-backend-residuals/`](../01-backend-residuals/README.md) step 6's D8-1/D8-2). It records the
shape here because 01 step 6 has to build the box, and the box's tag should be the **same identity**
this front puts in the erlang/beam value — otherwise `is Person` means two different things on two
backends. § 7 of [`options.md`](./options.md) says what wasm can store: not the atom (wasm has no
...
collision-checked, human-readable one is [A2](../16-module-naming/declaration-qualifier.md)'s atom.
...
| **§7 one source-shaped formatter per type** | the tag must **name the formatter** | A tag is enough if the backend also emits a lookup table. It is *more* than enough if the tag is the A2 atom under [policy 3](../16-module-naming/policy-3-module-per-type.md), because the atom is then a loadable module holding the type's functions: the formatter is `(maps:get('__bp_type', V)):format(V)`, one `call_ext`, no table, and a crash inside it names the type in the stack trace. **This is the only one of the four that forces the identity to be the module atom rather than any injective tag** — see [`front-16.md`](./front-16.md) § 2 |
...
`x is T` expression. [`../06-checker/`](../06-checker/README.md) N21 lands it, N20 the unions, N22
