...
| `lsp.md:104` `completion_decorator_record` | `usePost` is missing from its own completion list — correct, or a sign the degraded path drops `val` bindings? · **2026-09-17:** investigate first — the maintainer finds the fixture a weak example; find the real cause (degraded path dropping `val` bindings, or the decorator not recognised) and rewrite the test around it |
