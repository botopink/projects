...
program that declares a type with a method** (§ 7, E25). § 9 recommends cutting it out of front 16
into its own front for exactly those reasons.
...
unquoted per [E18](./evidence.md#e18--the-in-file-separator-candidates) —
...
bulk-accepted (the rule in [`../overview.md`](../overview.md#rules-carried-forward)).
...
([E26](./evidence.md#e26--local-call-vs-remote-call)).
...
([`options.md` A-flat](./options.md#the-output-layout-it-implies)):
...
`botopink run --target erlang` fails ([E25](./evidence.md#e25--botopink-run-breaks-under-policy-3)):
...
([E22](./evidence.md#e22--four-sibling-s-modules-from-one-source-file)):
...
and one of them hot-swaps alone ([E23](./evidence.md#e23--hot-swapping-one-type-module)). `beam_asm.zig`
...
Watch the quirk from [E17](./evidence.md#e17): `erlc +from_asm` prints a name/file mismatch but
...
**The maintainer decided that policy 3 runs inside front 16** ("16 e 16b fazer junto"). The steps
below are front 16's steps 7–13, the estimate is **≈ 9 days**, and the snapshot rule below is the
...
Front 16 as written is a **naming** front: it renames atoms, moves where files are written, and
...
rename and a re-shaping — and the rule carried forward from
[`../overview.md`](../overview.md#rules-carried-forward) is that a re-recorded snapshot is
...
module-atom carve-out the first half needs — it runs after 01 has closed, and 01's rows must not be
open when it starts.
