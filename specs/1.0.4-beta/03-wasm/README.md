...
**Status:** in flight since 2026-09-16 — worktree `.tasks/wasm`, branch `fix/wasm`, started from
`botopink-lang` `0552e32`. Scope unchanged from 1.0.2-beta front 06; the handoffs collected since
...
**Depends on:** nothing open — the 1.0.2-beta std-surface front has landed. W1's instance-method
and external sub-classes still reach `libs/std/src/primitives.bp`, which no backend front may edit:
a `libs/std` change is stopped and reported
...
### Step 6 — W7: the `@Option` none carrier (decided)

`@Option` none is the bare value `0`, indistinguishable from a real `0` (4 fixtures). This was a
**carrier decision, not a bug fix**, and it is taken:
[decision 3](../08-review-backlog/semantics-decisions.md#decision-3) (2026-09-16) rejects a sentinel
and boxes `?T` as an `i32` offset (`0` = null). Land W7 on that answer, and state it in
...
- [ ] W11 agrees with [`../02-erlang/README.md`](../02-erlang/README.md)'s E2 on what a non-string
      operand renders as
...
- **`if_simple_conditional_in_fn_body` is only half this front's.** wasm prints `256 0`: the `256` is
  ...
  [`../06-checker/README.md`](../06-checker/README.md)'s question. Do not re-record that second line.
...
|---|---|---|---|
| H1 | **Decision 3 answers W7**: box `?T` as a linear-memory `i32` offset, `0` = null; the sentinel reading is rejected. Step 6 is no longer a decision to take — implement it | [decision 3](../08-review-backlog/semantics-decisions.md#decision-3) (decided 2026-09-16) | step 6's acceptance, with the carrier stated in `src/codegen/AGENTS.md` as decided, not proposed |
| H2 | **`assert` is always fatal** — message and `file:line`; `wat.zig` has **no** `assert` lowering today. The message reaches stderr through the existing `fd_write` path, then `unreachable` | [decision 4](../08-review-backlog/semantics-decisions.md#decision-4) | `assert_*` fixtures trap with the message outside test mode (recorded as `RUNTIME TRAP (wasmtime):` once step 1 lands) |
| H3 | **The `executeWat` decision feeds [`../09-hygiene/`](../09-hygiene/README.md) step 2** (erase the removed WAT runtime). Whatever step 3 decides — execute, or a documented skip — write it into `src/codegen/AGENTS.md` in a form that front can quote; it does not start until it can | 1.0.2-beta hygiene | the decision is recorded and named in this front's landing note |
| H4 | **`$__print_*` follows decision 1** once `executeWat` runs: a string prints its text, a number its value, with no representation mapping | [decision 1](../08-review-backlog/semantics-decisions.md#decision-1) | wasm RUN LOGs of `@print` fixtures equal commonJS's under no mapping, except the documented numeric divergence |
