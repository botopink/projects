...
> **Carried from 1.0.2-beta comptime-dispatch step 2.** The erlang half (both fold paths, the
> `Args@1 = (Args ++ [X])` rebinding) landed with that front; what is left is the **beam** half,
> handed to this front as H3. The analysis below is kept whole because the beam fix needs the same
> shape. Line numbers were measured at the 1.0.2-beta commit — re-locate by symbol.
...
- **Backends:** erlang and beam_asm are broken; commonJS is correct by accident (JS arrays mutate
  ...
  ([`../01-beam/README.md`](../01-beam/README.md)), so this front either hands it the shape to fix
...
- **There is no notion of a mutating method anywhere in the compiler.** No marker in `ast.zig`,
  ...
  **without a return type** — `libs/std/src/primitives.bp:564-566`, `fn push(self: Self, item: T)` —
  ...
  (`std-surface` (1.0.2-beta, landed)); the name-driven version is
  self-contained here.
