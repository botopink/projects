...
|---|---|---|
| B10 | `if_simple_conditional_in_fn_body` | `val r = if (n > 0) { "positive"; };` with no `else` yields `undefined` on commonJS, `ok` on erlang, `undefined` on beam and `0` on wasm. Four backends, four answers to a question the language has not answered. Decide it in [`../06-checker/README.md`](../06-checker/README.md) and make all four agree |
