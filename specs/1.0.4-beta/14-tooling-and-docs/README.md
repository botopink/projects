...
- **Keyword token arrays**: `engine.zig:450,625,691,1040,1734,2187,3586` (compile-level changes land in 12; this front owns what they offer to the user).
...
- **botopink-lang markdown** teaching the syntax: 5 files, 29 occurrences — plus the defect that `docs.md` shows `implement A for B { … }` without `val` (fixed in the 1.0.2 surface by [`../09-hygiene/`](../09-hygiene/README.md) step 5; this front only
  checks it closed).
- The vscode-extension line numbers predate 1.0.2-beta 7e (snippets and grammar rewritten) and
  front 11 (the grammar's keyword pattern) — re-locate.
...
**Acceptance:**
- [ ] No `record`, `enum`, `interface` keyword left in the owned docs (manual verification)
