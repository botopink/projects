...
**Depends on:** [`../09-review-tooling/`](../09-review-tooling/README.md) — it changes the harness
in `src/comptime/tests/**` first (same files), and its `BOTOPINK_SNAP_TRACE` run is how step 1
proves no path was orphaned. **Must not run beside**
[`../07-checker/`](../07-checker/README.md) or [`../01-comptime-dispatch/`](../01-comptime-dispatch/README.md):
both re-record `snapshots/comptime/**`, the directory this front restructures
([`../fronts.md`](../fronts.md) note 3). Either order with 07-checker works; landing this front
first makes every checker regeneration cost a quarter of the files
...
([`../07-checker/`](../07-checker/README.md)) — the renderer reads inference, it does not change it
...
([`../09-review-tooling/`](../09-review-tooling/README.md)) · `src/codegen/**`

Paths are relative to `repository/botopink-lang/modules/compiler-core/`. Every `file:line` is at
`botopink-lang` HEAD, measured 2026-09-16.
...
[`../09-review-tooling/backlog.md`](../09-review-tooling/backlog.md) rows.
...
- [ ] A `BOTOPINK_SNAP_TRACE` run ([`../09-review-tooling/`](../09-review-tooling/README.md) step 1)
...
- [ ] The 17 tripwire slugs in [`renderer.md`](./renderer.md) still show `?` until
      [`../07-checker/`](../07-checker/README.md) resolves them — this front does not make them green
...
1. **`"id"`.** Two options:
   ...
   **Recommended: B**, unless a consumer of the id is found; A turns
   [`../03-std-surface/`](../03-std-surface/README.md)'s std edits into comptime re-records.
   Either way the `type_ids` plumbing in `src/comptime.zig:1276-1279`, `:1455-1458` becomes dead;
   removing it is a `src/comptime.zig` edit — register it with the front that owns that file rather
...
- **Other fronts.** [`../07-checker/`](../07-checker/README.md) sizes its rows in `?` counts and
  "68 files across the four dirs"
  ([`rows.md`](../07-checker/rows.md), [`blast-radius.md`](../07-checker/blast-radius.md)); after
  step 1 those are 17 files, after step 2 the syntactic `?`s its counts exclude are gone. Its
  counts must be re-measured, not scaled. [`../01-comptime-dispatch/primitive-surface.md`](../01-comptime-dispatch/primitive-surface.md)
  cites `snapshots/comptime/{node,erlang,beam,wasm}/`. [`../11-hygiene/`](../11-hygiene/README.md)'s
