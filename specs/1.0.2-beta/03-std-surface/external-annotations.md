...
[`node-externals.md`](./node-externals.md) (the 22 `./gleam_stdlib.mjs` rows and the shipping
decision), [`blockers.md`](./blockers.md) §6d (the import emitter) or
[`surface.md`](./surface.md) §E3 (`string:suffix/2`); it generalises them into one rule per case so
...
empty body lowers to `ok`; this is [`../04-beam/`](../04-beam/README.md) B2); wat skips the
...
measured). The primitive half is [`../01-comptime-dispatch/`](../01-comptime-dispatch/README.md)
...
([`../02-cli-gate/`](../02-cli-gate/README.md)).
...
| **R4** | Needs a host capability (clock, env, fs, process, crypto, regex, sockets) | **(a)** per target on a `declare fn`; wasm refuses at compile time (STD-001 with a location) until a WASI decision exists ([`../06-wasm/`](../06-wasm/README.md) W1) | `#[@External.Node("""process.cwd()"""), @External.Erlang("""(fun() -> {ok, P} = file:get_cwd(), list_to_binary(P) end)()""")] pub declare fn cwd() -> string;` |
