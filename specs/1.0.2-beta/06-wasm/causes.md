...
gaps, [`../07-checker/README.md`](../07-checker/README.md)), leaving 128 comparable:
...
the first line; the second belongs to [`../07-checker/README.md`](../07-checker/README.md) and is
...
([`../08-js-bridges/bridges.md`](../08-js-bridges/bridges.md)): a `loop` in value position. And
`loop_break_with_value`'s own expected value is unsettled — `fn find(arr) -> i32` returns a list
(beam's B7 note, [`../04-beam/README.md`](../04-beam/README.md)); settle the return type in the
...
| W11 | `narrow_else_if_chain_with_null_checks` | prints `nonzero:` and loses the `42` | a non-string operand dropped from a string concat — the wasm mirror of erlang E2 ([`../05-erlang/causes.md`](../05-erlang/causes.md)); agree on what a non-string operand renders as |
...
match them ([`../05-erlang/causes.md`](../05-erlang/causes.md#erlang-is-not-the-oracle)):
...
- **W1's instance-method and external sub-classes reach `libs/std/src/primitives.bp`**, which only
  [`../03-std-surface/README.md`](../03-std-surface/README.md) may edit. Land that front first; a
  lowering that needs a std change stops and reports.
- **W4 and JS-1, W11 and E2, W5 and E5/B1** are one language shape seen from several backends.
  ...
  re-records.
- **W7 is a representation choice.** Record it before any lowering changes, or two workers will pick
  two sentinels.
