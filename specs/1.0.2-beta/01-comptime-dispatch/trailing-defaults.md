...
The whole fix lands in `comptime/infer.zig`, which the checker front
([`../07-checker/README.md`](../07-checker/README.md)) owns and which moves the typed AST every
backend consumes. This front holds the analysis because the same library break drives it, but the
edit must be sequenced with the checker front — they cannot both hold `infer.zig` at once. See
