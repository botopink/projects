...
From [`../05-erlang/causes.md`](../05-erlang/causes.md#erlang-is-not-the-oracle). Do not "fix"
...
[`../07-checker/README.md`](../07-checker/README.md)'s to decide; do not re-record it here.

## The ranked table

| # | Cause | Closes | Owner |
|---|---|---|---|
| C1 | The std string/array methods name `./gleam_stdlib.mjs`, which exists nowhere | **9** (aborts) | [`../03-std-surface/`](../03-std-surface/README.md) — not this front |
...
## C1 — `./gleam_stdlib.mjs` (9 fixtures) — not this front's

`libs/std/src/primitives.bp` names the file 22 times, it is the Gleam language's runtime, and the
decision is to remove the dependency rather than ship it. The table of the 22 declarations, which 4
break, and the fix all live in
[`../03-std-surface/node-externals.md`](../03-std-surface/node-externals.md); the step is
[`../03-std-surface/README.md`](../03-std-surface/README.md) step 2. It needs no compiler change —
except the helper mechanism for primitives with no matching native method, which that front routes
here (see [`README.md`](./README.md) Notes).
