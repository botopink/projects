...
- `throw_inside_case_arm`'s `return return` is not a JS-lowering bug on its own — it is the
  ...
  as `{ok, {error, …}}` ([`../05-erlang/causes.md`](../05-erlang/causes.md), E8). Fix the wrap once,
...
- `loop_break_with_value`'s expected value is itself unsettled: `fn find(arr) -> i32` returns a
  list. Settle the return type in [`../07-checker/README.md`](../07-checker/README.md) before
...
The same language shape is wasm's W4 ([`../06-wasm/causes.md`](../06-wasm/causes.md)) and beam's B7
([`../04-beam/causes.md`](../04-beam/causes.md)).
...
[`../07-checker/README.md`](../07-checker/README.md)'s parser-gap step. **Blocked on that parser gap.**
...
also has to give a real node ([`../05-erlang/raw-rows.md`](../05-erlang/raw-rows.md)). Decide the
