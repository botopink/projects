# Cross-backend semantics — the four decisions
...
[`../03-std-surface/`](../03-std-surface/README.md)).

The per-backend fronts already compare RUN LOGs under a **representation mapping** that undoes `~p`
(`<<"x">>` → `x`, `1.0` → `1`, `[ 2, 4 ]` → `[2,4]`; see
[`../06-wasm/causes.md`](../06-wasm/causes.md) and its beam/erlang twins). That mapping is the
...
- Once `executeWat` exists ([`../06-wasm/`](../06-wasm/README.md)), wasm's `$__print_*` family must
  match the same rule.
...
- [`../07-checker/README.md`](../07-checker/README.md) step 8 (narrowing, B7): its "identical RUN
...
[`../07-checker/`](../07-checker/README.md)), so it is a real tripwire and not renderer noise (see
[`../10-comptime-dedup/renderer.md`](../10-comptime-dedup/renderer.md)).
...
[`../07-checker/rows.md`](../07-checker/rows.md)).
...
- The checker rows in [`../07-checker/`](../07-checker/README.md) that decide how a block and a
...
**Conflict to resolve before either front starts:** [`../06-wasm/README.md`](../06-wasm/README.md)
step 6 (W7, "decide the `@Option` none carrier") and [`../06-wasm/causes.md`](../06-wasm/causes.md)
...
file owns the rule. Agree one answer and have 06-wasm's step 6 cite it.
...
- The four W7 fixtures in [`../06-wasm/causes.md`](../06-wasm/causes.md):
...
- Verification of any of them needs `executeWat` ([`../06-wasm/`](../06-wasm/README.md)).
...
[`../02-cli-gate/`](../02-cli-gate/README.md)). If B is chosen instead, `assert` must be removed from
