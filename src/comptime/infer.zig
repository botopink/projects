...
fn appendAnnotationsHandle(arena: std.mem.Allocator, anns: []const ast.Annotation) ![]const decoratorEval.DeclHandle.Annotation {
    var result = try arena.alloc(decoratorEval.DeclHandle.Annotation, anns.len);
    var count: usize = 0;
    for (anns) |a| {
        if (a.is_builtin) continue;
        result[count] = .{ .name = a.name, .args = a.args };
        count += 1;
    }
    return result[0..count];
}

fn appendParamsHandle(arena: std.mem.Allocator, params: []const ast.Param) ![]const decoratorEval.DeclHandle.Param {
    var result = try arena.alloc(decoratorEval.DeclHandle.Param, params.len);
    for (params, 0..) |p, i| {
        result[i] = .{ .name = p.name, .typeName = declTypeName(p.typeRef) };
    }
    return result;
}

fn appendMethodsHandle(arena: std.mem.Allocator, methods: []const ast.InterfaceMethod) ![]const decoratorEval.DeclHandle.Method {
    var result = try arena.alloc(decoratorEval.DeclHandle.Method, methods.len);
    for (methods, 0..) |m, i| {
        result[i] = .{
            .name = m.name,
            .params = try appendParamsHandle(arena, m.params),
            .returnType = if (m.returnType) |rt| declTypeName(rt) else "",
            .annotations = try appendAnnotationsHandle(arena, m.annotations),
        };
    }
    return result;
}
...
) !decoratorEval.DeclHandle {
    var handle_fields = try arena.alloc(decoratorEval.DeclHandle.Field, fields.len);
    for (fields, 0..) |f, i| {
        handle_fields[i] = .{
            .name = f.name,
            .typeName = f.typeName,
            .annotations = try appendAnnotationsHandle(arena, f.annotations),
        };
    }
    return decoratorEval.DeclHandle{
        .kind = kind,
        .name = name,
        .fields = handle_fields,
        .methods = try appendMethodsHandle(arena, methods),
        .returnType = returnType,
        .annotations = try appendAnnotationsHandle(arena, annotations),
    };
}
...
fn invokeDecorators(env: *Env, program: ast.Program) InferError!void {
    ...
    for (program.decls) |decl| switch (decl) {
        .@"fn" => |f| {
            const h = try buildHandle(env.arena, "Fn", f.name, &.{}, &.{}, if (f.returnType) |rt| declTypeName(rt) else "", f.annotations);
        ...
        .record => |r| {
            ...
            const h = try buildHandle(env.arena, "Record", r.name, fields, r.methods, "", r.annotations);
            try runDeclDecorators(env, ctx, r.annotations, h);
            for (r.fields) |fld| {
                const fh = try buildHandle(env.arena, "Field", fld.name, &.{}, &.{}, declTypeName(fld.typeRef), fld.annotations);
                try runDeclDecorators(env, ctx, fld.annotations, fh);
            }
            for (r.methods) |m| {
                const mh = try buildHandle(env.arena, "Method", m.name, &.{}, &.{}, if (m.returnType) |rt| declTypeName(rt) else "", m.annotations);
                try runDeclDecorators(env, ctx, m.annotations, mh);
            }
        },
        .@"enum" => |e| {
            const h = try buildHandle(env.arena, "Enum", e.name, &.{}, e.methods, "", e.annotations);
        ...
        .interface => |i| {
            ...
            const h = try buildHandle(env.arena, "Interface", i.name, fields, i.methods, "", i.annotations);
...
fn expandTemplateCallViaRuntime(
    ...
    const expansion: *const ast.Expr = switch (outcome) {
        ...
        .custom => |c| blk: {
            ...
            // otherwise convert from native CustomNodeTree.
