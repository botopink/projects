...
| `eval.zig` | Builds eval scripts, calls runtime, parses results via native structs. |
...
| `template.zig` | `@Expr` template infrastructure: `CapturedExpr` (an argument bound to a `comptime p: @Expr<T>` param, captured unevaluated with provenance), `PlainArg` (a non-`@Expr` param that received a literal value at the call site — emitted as a plain Erlang binding in the eval script), `ScopeSnapshot` (V1 origin scope: caller's top-level decls + imports, serializable via `toJsonAlloc`), `contextJsonAlloc` (the full second-layer handle — **TODO:** replace with `buildCaptureContext()` returning native `CaptureContext` struct), and `mapSpanToLoc`/`failDiagnostic` (rustc-style `fail`/`failAt` diagnostics pointing inside the caller's `"""…"""`). `mapSpanToLoc` fallback for holed templates uses `capture.loc.line + span.line - 1` (span.line is 1-based, line 1 = opening `"""` line). |
| `template_eval.zig` | F6-full: runs a non-V1 template body in the **Erlang** eval runtime (host-side comptime, independent of the compile target). Captures become Erlang maps implementing the comptime surface (`text`/`parts`/`source`/`context`/`lookup`/`bindings`/`build`/`fail`/`failAt`) over the `contextJsonAlloc` handle; plain args are emitted as Erlang bindings before capture objects; params are passed in declaration order. The script reports one protocol result — `code` (parse + splice), `value` (`@expr` lift → literal), `capture` (param pass-through), `fail` (template diagnostic), `error` — via JSON (parsed automatically into `TemplateResult` struct by `std.json.parseFromSliceLeaky`). **TODO:** Migrate from `std.json.Value` to native `TypedValue` struct for type safety. |
| `decorator_eval.zig` | Annotation processors (P2): runs a decorator body in the **Erlang** eval runtime over the declaration it annotates. Uses native `DeclHandle` struct (no JSON intermediate format). The `DeclHandle` is emitted directly as Erlang terms in the generated module. Each `fields[]` entry carries its own `name`/`typeName`/**`annotations`** (so a record decorator reads per-field markers like `#[value]`/`#[inject]`), and each `methods[]` entry its `params`/`returnType`/`annotations` (so a controller decorator reads `#[getMapping]`/… off methods). The script reports `ok` (placement accepted), `fail` (scoped diagnostic), or `error` via JSON (parsed automatically into `ErlResult` struct by `std.json.parseFromSliceLeaky`). Driven by `infer.zig invokeDecorators`, which builds `DeclHandle` native structs and surfaces a `fail` as a `TypeError`. Erlang-only, skipped in tooling paths. |
...
F10) — computed objects come back through `valueToAstLiteral(TypedValue)` for type safety. Remaining
limits (recorded): all params must be `@Expr` captures; Erlang runtime only.
...
- **P2 invocation:** `invokeDecorators` (right after `validateDecorators`) runs
  each body-carrying decorator over the declaration it annotates. `buildHandle`
  builds a native `DeclHandle` struct (kind/name/fields/methods/returnType/
  annotations); `decorator_eval.zig` emits the body as Erlang terms directly from
  the AST + `DeclHandle`, runs it in the **Erlang** runtime, and reports
  `ok` / `fail{message,span}` / `error` via JSON (parsed automatically into
  `ErlResult` struct). A `fail`/`failAt` becomes a scoped
